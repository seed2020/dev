...
/** 파일 미리보기(문서뷰어) */
@Controller
public class MoEmFileUploadCtrl {
	
	/** Logger */
	private static final Logger LOGGER = Logger.getLogger(MoEmFileUploadCtrl.class);
	...
	public String setUploadProgressPop(HttpServletRequest request,
		...
		return MoLayoutUtil.getJspPath("/em/setUploadProgressPop");
	}
	
	/** [AJAX] 이미지 업로드 */
	@RequestMapping(value = "/cm/transImgUploadAjx", method = RequestMethod.POST)
	public String transImgUploadAjx(HttpServletRequest request,
		...
		try{
			...
			if(file!=null){
				...
				BufferedImage bimg = ImageIO.read(new File(filePath));
				//if(callback==null || callback.isEmpty()) callback = "setImage";
				//model.put("todo", callback+"('"+emTmpFileTVo.getTmpFileId()+"', "+Integer.toString(bimg.getWidth())+", "+Integer.toString(bimg.getHeight())+", "+emTmpFileTVo.getFileSize()+");");
				model.put("imgData", "{\"tmpFileId\":\""+emTmpFileTVo.getTmpFileId()+"\", \"wdth\":\""+Integer.toString(bimg.getWidth())+"\", \"hght\":\""+Integer.toString(bimg.getHeight())+"\", \"size\":\""+emTmpFileTVo.getFileSize()+"\"}");
	...
	public String viewFileListPop(HttpServletRequest request,
		...
		if(request.getRequestURI().startsWith("/bb")){
			...
			model.put("module", "bb");
		}else if(request.getRequestURI().startsWith("/dm")){
			...
			model.put("module", "dm");
		}else if(request.getRequestURI().startsWith("/ct")){
			String bullId=ParamUtil.getRequestParam(request, "bullId", true);
			paramMap.put("refTyp", "ct");
			paramMap.put("refId", bullId);
			model.put("module", "ct");
		}else if(request.getRequestURI().startsWith("/wl")){
			String logNo=ParamUtil.getRequestParam(request, "logNo", false);
			paramMap.put("refTyp", "wl");
			paramMap.put("refId", logNo);			
			model.put("module", "wl");
		}else if(request.getRequestURI().startsWith("/wh")){
			...
			model.put("module", "wh");
		}else if(request.getRequestURI().startsWith("/wf")){
			...
			return MoLayoutUtil.getJspPath("/em/viewFileListPop");
		}
		...
		if(isFrm){// 프레임
			...
			return MoLayoutUtil.getJspPath("/em/viewFileListPop","Frm");
		}
