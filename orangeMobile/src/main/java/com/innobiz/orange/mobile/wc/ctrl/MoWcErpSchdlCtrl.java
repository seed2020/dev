...
/** 일정관리 */
@Controller
public class MoWcErpSchdlCtrl {
	/** Logger */
	private static final Logger LOGGER = Logger.getLogger(MoWcErpSchdlCtrl.class);
    ...
    /** 일정 보기 */
	@RequestMapping(value = "/wc/pub/listCalendar")
	public String listCalendar(HttpServletRequest request,
		...
		try{
			...
			/** 일정구분코드 조회 */
			List<PtCdBVo> wcCatClsBVoList = ptCmSvc.getCdList("ERP_SCHDL_CD", langTypCd, "Y");
		...
		return MoLayoutUtil.getJspPath("/wc/pub/listCalendar","wc");
	}
	/** 상세보기 */
	@RequestMapping(value = "/wc/pub/viewSchdl")
	public String viewSchdl(HttpServletRequest request,
		...
		wcErpSchdlBVo.setQueryLang(langTypCd);
		wcErpSchdlBVo.setSchdlId(schdlId);
		wcErpSchdlBVo = (WcErpSchdlBVo)commonSvc.queryVo(wcErpSchdlBVo);

		model.put("wcErpSchdlBVo", wcErpSchdlBVo);
		model.put("paramsForList", ParamUtil.getQueryString(request, "menuId","schdlId","noCache"));

		return MoLayoutUtil.getJspPath("/wc/pub/viewSchdl");
	}
    ...
    /** [AJAX] 일정 조회 */
	@RequestMapping(value = "/wc/pub/listSchdlAjx")
	public String listSchdlAjx(HttpServletRequest request,
		...
		try{
			...
			// 조회
			WcErpSchdlBVo wcErpSchdlBVo = new WcErpSchdlBVo();
			wcErpSchdlBVo.setCompId(userVo.getCompId());
			
			//일정 조회
			wcErpSchdlBVo.setStrtDt(realStrtDt);
			wcErpSchdlBVo.setEndDt(end);
			if(schdlTypCd!=null && !schdlTypCd.isEmpty()) wcErpSchdlBVo.setSchdlTypCd(schdlTypCd);
			if(schUserUid!=null && !schUserUid.isEmpty()) wcErpSchdlBVo.setUserUid(schUserUid);
			
			@SuppressWarnings("unchecked")
			List<WcErpSchdlBVo> wcErpSchdlBVoList = (List<WcErpSchdlBVo>)commonSvc.queryList(wcErpSchdlBVo);
			model.put("wcSchdlBVoList", wcErpSchdlBVoList);
	...
	@RequestMapping(value = "/wc/pub/setSchdl")
	public String setSchdlPop(HttpServletRequest request,
		...
		if(schdlId != null && !"".equals(schdlId)){
			...
			//시작일자 세팅
            wcErpSchdlBVo.setStrtDt(new Timestamp(cal.getTimeInMillis()).toString());
            cal.add(Calendar.HOUR_OF_DAY, 1);
            //종료일자 세팅
            wcErpSchdlBVo.setEndDt(new Timestamp(cal.getTimeInMillis()).toString());
		...
		return MoLayoutUtil.getJspPath("/wc/pub/setSchdl");
	}
	
	/** 일정 등록, 수정*/
	@RequestMapping(value = "/wc/pub/transSchdlPost")
	public String transSchdl(HttpServletRequest request,
		...
		try {
			...
			WcErpSchdlBVo wcErpSchdlBVo = new WcErpSchdlBVo();
			VoUtil.bind(request, wcErpSchdlBVo);
			wcErpSchdlBVo.setCompId(userVo.getCompId());
			wcErpSchdlBVo.setStrtDt(startYmd);
			wcErpSchdlBVo.setEndDt(startYmd);
			
			if(schdlId==null || schdlId.isEmpty()){
				wcErpSchdlBVo.setSchdlId(wcCmSvc.createId("WC_ERP_SCHDL_B"));
				wcErpSchdlBVo.setAlldayYn("Y");
				// 등록자, 등록일시
				wcErpSchdlBVo.setRegrUid(userVo.getUserUid());
				wcErpSchdlBVo.setRegDt("sysdate");
				commonSvc.insert(wcErpSchdlBVo);
			}else{
				// 수정자, 수정일시
				wcErpSchdlBVo.setModrUid(userVo.getUserUid());
				wcErpSchdlBVo.setModDt("sysdate");
				commonSvc.update(wcErpSchdlBVo);
			}
	...
	/** [AJAX] 일정 저장 */
	@RequestMapping(value = "/wc/pub/transSchdlAjx")
	public String transSchdlAjx(HttpServletRequest request,
		...
		try {
			...
			wcErpSchdlBVo.setCompId(userVo.getCompId());
			wcErpSchdlBVo.setSchdlId(schdlId);
			wcErpSchdlBVo.setStrtDt(schdlStartDt);
			wcErpSchdlBVo.setEndDt(schdlEndDt);
			wcErpSchdlBVo.setAlldayYn(alldayYn);
			commonSvc.update(wcErpSchdlBVo);
	...
	/** [AJAX] 일정 삭제 */
	@RequestMapping(value = "/wc/pub/transSchdlDelAjx")
	public String transSchdlDelAjx(HttpServletRequest request,
		...
		try {
			...
			commonSvc.delete(wcErpSchdlBVo);
