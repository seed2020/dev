...
%>
<u:set var="callback" test="${!empty param.callback }" value="${param.callback }" elseValue="setFldInfos"/>
...
function fldTree(tree){
	...
	$m.ajax('${_cxPth}/dm/doc/treeFldAjx.do?menuId=${menuId}&fldId=${param.fldId }', null, function(data){
		if(data.openLvl != null) tree.openLvl = data.openLvl;
		var i, objVo, objArr = data.dmFldBVoList;
		for(i=0;i<objArr.length;i++){
			objVo = objArr[i];
			tree.add(objVo.fldPid, objVo.fldId, objVo.fldNm, 'F', '0'+objVo.sortOrdr, objVo.rescId, {id:objVo.fldId, nm:objVo.fldNm, pid:objVo.fldPid, storId:objVo.storId, rescId:objVo.rescId, useYn:objVo.useYn, title:objVo.fldId});
		}
		tree.draw();
