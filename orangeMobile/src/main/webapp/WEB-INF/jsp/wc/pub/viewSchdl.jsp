...
function setSchdl(schdlId){
	...
	var url = "/wc/pub/setSchdl.do?menuId=${menuId}&${paramsForList }";
	if(schdlId != null) {
		url+= "&schdlId="+schdlId;
	}
...
function fnDelete(flag,schdlId){
	$m.msg.confirmMsg("cm.cfrm.del", null , function(result){
		if(result){
			$m.ajax('/wc/pub/transSchdlDelAjx.do?menuId=${menuId}', {schdlId:schdlId}, function(data) {
...
<% // 목록으로 이동 %>
function goList() {
	$m.nav.prev(event, '/wc/pub/listCalendar.do?menuId=${menuId}&${paramsForList}');
}
...
<section>
       ...
       <div class="btnarea" id="btnArea">
           <div class="size">
           <dl>
		       <u:secu auth="A"><dd class="btn" onclick="setSchdl('${wcErpSchdlBVo.schdlId }');"><u:msg titleId="cm.btn.mod" alt="수정" /></dd>
		       <dd class="btn" onclick="fnDelete('del','${wcErpSchdlBVo.schdlId }');"><u:msg titleId="cm.btn.del" alt="삭제" /></dd></u:secu>
			...
			<div  class="s_tablearea">
				...
				<table class="s_table">
					...
					<tr>
						...
						<td class="shead_lt"><u:out value="${wcErpSchdlBVo.schdlTypNm }" /></td>
					</tr>
					<tr>
						<th class="shead_lt"><u:msg titleId="wc.cols.schdlKndCd" alt="일정대상"/></th>
						<td class="shead_lt"><u:out value="${wcErpSchdlBVo.userNm }" /></td>
					</tr>
					<tr>
						<th class="shead_lt"><u:msg titleId="cols.subj" alt="제목"/></th>
						<td class="shead_lt"><u:out value="${wcErpSchdlBVo.subj }" /></td>
					</tr>
					<tr>
						<th class="shead_lt"><u:msg titleId="wc.cols.date" alt="일자"/></th>
						<td class="shead_lt"><u:out value="${wcErpSchdlBVo.strtDt}" type="date"/></td>
					</tr>
					<tr>
						<th class="shead_lt"><u:msg titleId="cols.cont" alt="내용" /></th>
						<td class="shead_lt"><textarea rows="3" id="cont" name="cont" class="etr_ta"  onfocus="$('#focusLayout').show();" onblur="$('#focusLayout').hide();" style="width:95%;">${wcErpSchdlBVo.cont}</textarea></td>
					</tr>
				</tbody>
			</table>
		</div>
