...
function save(){
	...
	$m.msg.confirmMsg("cm.cfrm.save", null, function(result){
		if(result){
			var $form = $('#setRegForm');
			$form.attr('action','/wc/pub/transSchdlPost.do?menuId=${menuId}');
			$m.nav.post($form);
		}
	});
};
...
function openSingleUser(areaNm){
	$m.user.selectOneUser({userUid:'${wcErpSchdlBVo.userUid}'}, function(userVo){
		if(userVo==null){<%
...
<section>
	...
	<form id="setRegForm" name="setRegForm" >
    	...
    	<input type="hidden" id="listPage" name="listPage" value="/wc/pub/listCalendar.do?menuId=${menuId}&${paramsForList}" />
    	<input type="hidden" id="viewPage" name="viewPage" value="/wc/pub/viewSchdl.do?menuId=${menuId}&${viewPageParams}" />
    	
    	<!-- 일정종류 -->
    	<u:set var="schdlTypCd" test="${empty wcErpSchdlBVo.schdlId && !empty wcCatClsBVoList }" value="${wcCatClsBVoList[0].cd }" elseValue="${wcErpSchdlBVo.schdlTypCd }"/>
	    ...
	    <div class="entryzone">
	        <div class="entryarea">
	        <dl>
		        <dd class="etr_tit"><u:msg titleId="wc.btn.schdl${!empty wcErpSchdlBVo.schdlId ? 'Mod' : 'Reg'}" alt="일정 등록/수정" /></dd>
		        ...
		        <dd class="etr_select">
                        ...
                        <div class="open_in1">
                            ...
                            <dl>
                            	<c:forEach  var="list" items="${wcCatClsBVoList}" varStatus="status">
                            		...
                            		<c:if test="${!empty wcErpSchdlBVo.schdlId && wcErpSchdlBVo.schdlTypCd == list.cd}"><c:set var="schdlTypNm" value="${list.rescNm}"/></c:if>
                    ...
                    <c:if test="${ empty wcErpSchdlBVo.schdlId && !empty wcCatClsBVoList}"><c:set var="schdlTypNm" value="${wcCatClsBVoList[0].rescNm}"/></c:if>
                ...
                <dd class="etr_input">
					...
					<dd class="etr_ip_lt">
						<div id="ownrInfoArea" >
							<div id="idArea" style="display:none;"><input type="hidden" id="userUid" name="userUid" value="${wcErpSchdlBVo.userUid }"/></div>
							<div id="nmArea" style="display:inline;"><m:input type="text" id="userNm" name="userNm" className="etr_iplt" titleId="wc.cols.schdlKndCd" value="${wcErpSchdlBVo.userNm }" style="width:55%;" readonly="Y" mandatory="Y"/></div>
		              	</div>
					</dd>
                ...
                <dd class="etr_input"><div class="etr_inputin"><input type="text" class="etr_iplt" name="subj" id="subj" value="${wcErpSchdlBVo.subj }"/></div></dd>
                <dd class="etr_bodytit_asterisk"><u:msg titleId="wc.cols.date" alt="일자"/></dd>
		        <dd class="etr_select"><fmt:parseDate var="dateStartDt" value="${wcErpSchdlBVo.strtDt}" pattern="yyyy-MM-dd HH:mm:ss"/>
		        ...
		        <dd class="etr_bodytit"><u:msg titleId="cols.cont" alt="내용" /></dd>
           		<dd class="etr_input" ><div class="etr_textareain"><textarea rows="3" id="cont" name="cont" class="etr_ta"  onfocus="$('#focusLayout').show();" onblur="$('#focusLayout').hide();">${wcErpSchdlBVo.cont}</textarea></div></dd>
        	</dl>
    	</div>
 	</div>
