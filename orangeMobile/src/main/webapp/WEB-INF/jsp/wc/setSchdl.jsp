...
function chkOpenGradCd(obj){
	...
	if(val=='4'){
		$('#deptBtnArea dd.btn').show();
	}else{
		deptSelectDel();
		$('#deptBtnArea dd.btn').hide();
	}		
}<% // [버튼클릭] - 부서삭제  %>
...
function addDept(){
	...
	$.each($('#deptSelectArea input[name="orgId"]'), function(){
		if($(this).val()!='')
			data.push(JSON.parse('{"orgId":"'+$(this).val()+'","rescNm":"'+$(this).closest('dd.title').text()+'"}'));
	});
...
function createUbox(idVa, orgNm, isGrp){
	...
	buffer.push('</dd>');
	buffer.push('<dd class="btn" onclick="deptSelectDel(this);"></dd>');
	buffer.push('</dl></div>');
	return buffer.join('');
}<% // 부서 목록  %>
...
function guestAdd(){
	...
	if(guestTyp == 'FRND'){
		...
		$m.nav.next(null, '/wc/findUserGrp.do?menuId=${param.menuId}&fncCal=${fncCal}&selection=multi${queryParamUserUid}');
	}else{
		openMuiltiUser();
	}
}
...
function fnCalendar(id,opt,hm,hmId,handler){
	$m.dialog.open({
		id:id,
		noPopbody:true,
		url:'/cm/util/getCalendarPop.do?&id='+id+'&opt='+opt+'&val='+$('#'+id).val()+'&hm='+hm+'&hmId='+hmId+'&hmVal='+$('#'+hmId).val()+'&handler='+handler+'&hmUnit=Y',
	});
};
...
function setOptionVal(id , code, value){
	var $form = $("#setRegForm");
	if(id == 'grpId'){// 그룹일정 일 경우에만 그룹선택 활성화
		if($form.find("input[name='schdlKndCd']").val() != '2'){
			return;
		}
	}
	if(id == 'schdlKndCd'){
		if(code=='2'){
			$('#grpIdSelectContainer').show();
		}else{
			$('#grpIdSelectContainer').hide();
			setOptionVal('grpId','',"<u:msg titleId="cm.option.all" alt="전체선택"/>");
		}
	}
	...
	$m.dialog.openSelect({id:'repetSelectPop', cdList:getRepetOpts(), selected:''}, function(selObj){
		if(selObj!=null){
			if(selObj.cd =="N") {
				...
				var url = '/wc/repetSelectSub.do?menuId=${menuId}&fncCal=${fncCal}&repetKnd='+selObj.cd;
				...
				/* $m.dialog.open({
					...
					url:'/wc/repetSelectPop.do?menuId=${menuId}&repetKnd='+selObj.cd,
				}); */
			}
		}else{
			chkFileSetting();
...
function save(){
	...
	if(startDt==endDt){
		...
		//alert($('#setRegForm').find('input[name="alldayYn"]:checked').val());
	...
	$m.msg.confirmMsg("cm.cfrm.save", null, function(result){
		if(result){
			...
			var $form = $('#setRegForm');
			$form.attr('action','/wc/'+transPage+'.do?menuId=${menuId}&fncCal=${fncCal}');
			$m.nav.post($form);
		}
	});
};
...
<div class="btnarea">
    <div class="size">
        <dl>
            ...
            <u:secu auth="W"><dd class="btn" onclick="save();"><u:msg titleId="cm.btn.save" alt="저장" /></dd></u:secu>
     </dl>
    </div>
</div>
<section>
	...
	<form id="setRegForm" name="setRegForm" enctype="multipart/form-data" action="/wc/${transPage}Post.do?menuId=${menuId}">
    	...
    	<textarea id="cont" name="cont" style="display:none">${wcSchdlBVo.cont}</textarea>
    	
    	<!-- 일정종류 -->
    	<u:set var="schdlTypCd" test="${empty wcSchdlBVo.schdlId && !empty wcCatClsBVoList }" value="${wcCatClsBVoList[0].catId }" elseValue="${wcSchdlBVo.schdlTypCd }"/>
		...
		<input type="hidden" id="listPage" name="listPage" value="/wc/${listPage }.do?menuId=${menuId}&${paramsForList}" />
		<input type="hidden" id="viewPage" name="viewPage" value="/wc/viewSchdl.do?menuId=${menuId}&schdlId=${wcSchdlBVo.schdlId}&fncCal=${fncCal}&tabNo=${tabNo}" />
	    ...
	    <div class="entryzone">
	        <div class="entryarea">
	        <dl>
		        <dd class="etr_tit"><u:msg titleId="wc.btn.schdl${!empty wcSchdlBVo.schdlId ? 'Mod' : 'Reg'}" alt="일정 등록/수정" /></dd>
		        ...
		        <dd class="etr_select">
                        ...
                        <div class="open_in1">
                            ...
                            <dl>
                            	<c:forEach  var="list" items="${wcCatClsBVoList}" varStatus="status">
                            		...
                            		<c:if test="${!empty wcSchdlBVo.schdlId && wcSchdlBVo.schdlTypCd == list.catId}"><c:set var="schdlTypNm" value="${list.catNm}"/></c:if>
                    ...
                    <c:if test="${ empty wcSchdlBVo.schdlId && !empty wcCatClsBVoList}"><c:set var="schdlTypNm" value="${wcCatClsBVoList[0].catNm}"/></c:if>
                ...
                <c:choose>
                	<c:when test="${param.fncCal eq 'my' || param.fncCal eq 'open' || wcSchdlBVo.schdlKndCd eq '1' || wcSchdlBVo.schdlKndCd eq '2'}">
                		<dd class="etr_input">
		                    <div class="etr_ipmany">
		                    	<dl>
				                    <dd class="etr_se_lt">
				                        <div class="etr_open2" id="schdlKndCdContainer" style="display:none">
				                            <div class="open_in1">
				                                <div class="open_div">
				                                <dl>
				                                	...
				                                	<c:forEach  var="list" items="${schdlKndCdList}" varStatus="status">
														<c:if test="${list[2] eq 'Y'}">
															...
															<c:if test="${empty schdlKndNm || !empty wcSchdlBVo.schdlId && wcSchdlBVo.schdlKndCd == list[0]}">
																<c:set var="schdlKndNm" value="${list[1]}"/><c:set var="schdlKndCd" value="${list[0]}"/>
															</c:if>
															<dd class="txt" onclick="setOptionVal('schdlKndCd',$(this).attr('data-code'),$(this).text());" data-code="${list[0] }">${list[1] }</dd>
		                        		...
		                        		<div class="select_in1" onclick="$('#setRegForm #schdlKndCdContainer').toggle();">
					                        <dl>
				                        		<dd class="select_txt" id="schdlKndCdView" ><span>${schdlKndNm }</span></dd>
				                        		<dd class="select_btn"><input type="hidden" id="schdlKndCd" name="schdlKndCd"  value="${schdlKndCd }"/></dd>
					                        </dl>
		                        		</div>
		                    		</dd>
		                    		<u:set var="grpIdSelectStyle" test="${wcSchdlBVo.schdlKndCd eq '2' }" value="" elseValue="display:none;"/>
		                    		<dd class="etr_se_rt" id="grpIdSelectContainer" style="${grpIdSelectStyle}">
				                        <div class="etr_open3" id="grpIdContainer" style="display:none">
				                            <div class="open_in2">
				                                <div class="open_div">
				                                <dl>
				                                	<dd class="txt" onclick="setOptionVal('grpId',$(this).attr('data-code'),$(this).text());" data-code=""><u:msg titleId="cm.option.all" alt="전체선택"/></dd>
						                        	<dd class="line"></dd>
				                                	<c:forEach  var="grpItem" items="${wcSchdlGroupBVoList}" varStatus="status">
				                                		<c:if test="${status.index > 0 && fn:length(wcSchdlGroupBVoList) > status.count }"><dd class="line"></dd></c:if>
				                                		<c:if test="${!empty wcSchdlBVo.schdlId && wcSchdlBVo.grpId == grpItem.schdlGrpId}"><c:set var="grpNm" value="${grpItem.grpNm}"/><c:set var="grpId" value="${grpItem.schdlGrpId}"/></c:if>
						                        		<dd class="txt" id="grpIdView" onclick="setOptionVal('grpId',$(this).attr('data-code'),$(this).text());" data-code="${grpItem.schdlGrpId }">${grpItem.grpNm }</dd>
						                        	</c:forEach>
					                            </dl>
				                                </div>
				                            </div>
				                        </div>
		                        		<div class="select_in2" onclick="$('#setRegForm #grpIdContainer').toggle();">
		                        			<dl>
				                        		<dd class="select_txt" id="grpIdView"><span><c:choose><c:when test="${!empty grpNm }">${grpNm }</c:when><c:otherwise><u:msg titleId="cm.option.all" alt="전체선택"/></c:otherwise></c:choose></span></dd>
				                        		<dd class="select_btn"><input type="hidden" id="grpId" name="grpId"  value="${grpId }"/></dd>
		                      			   </dl>
		                  				</div>
		                			</dd>
		                		</dl>
                	...
                	<c:when test="${param.fncCal eq 'dept' || wcSchdlBVo.schdlKndCd eq '3'}"><dd class="etr_input"><u:msg titleId="wc.jsp.listPsnSchdl.dept.title" alt="부서일정"/><input type="hidden" name="schdlKndCd"  value='3'/></dd></c:when>
                	<c:when test="${param.fncCal eq 'comp' || wcSchdlBVo.schdlKndCd eq '4'}"><dd class="etr_input"><u:msg titleId="wc.jsp.listPsnSchdl.comp.title" alt="회사일정"/><input type="hidden" name="schdlKndCd"  value='4'/></dd></c:when>
                </c:choose>
                
                <dd class="etr_bodytit_asterisk"><u:msg titleId="cols.subj" alt="제목" /></dd>
                <dd class="etr_input"><div class="etr_inputin"><input type="text" class="etr_iplt" name="subj" id="subj" value="${wcSchdlBVo.subj }"/></div></dd>
                <dd class="etr_bodytit"><u:msg titleId="cols.loc" alt="장소" /></dd>
                <dd class="etr_input"><div class="etr_inputin"><input type="text" class="etr_iplt" name="locNm" id="locNm" value="${wcSchdlBVo.locNm}"/></div></dd>
                
                <u:out value="${wcSchdlBVo.schdlStartDt}" type="date" var="schdlStrtYmd" />
				<u:out value="${wcSchdlBVo.schdlStartDt}" type="hm" var="schdlStrtTime" />
				<u:out value="${wcSchdlBVo.schdlEndDt}" type="date" var="schdlEndYmd" />
				<u:out value="${wcSchdlBVo.schdlEndDt}" type="hm" var="schdlEndTime" />
                ...
                <dd class="etr_input">
                    <div class="etr_ipmany">
                    <dl>
                    	<u:set var="checked" test="${wcSchdlBVo.alldayYn == 'Y'}" value="true" elseValue="false"/>
						<m:check type="checkbox" id="alldayYn" name="alldayYn" inputId="alldayYn" value="Y" checked="${checked }" titleId="wc.cols.alldayYn" />
						
						<u:set var="checked" test="${wcSchdlBVo.solaLunaYn == 'N'}" value="true" elseValue="false"/>
		        ...
		        <dd class="etr_input">
                    <div class="etr_ipmany" id="openGradContainer">
                    <dl>
                    	<m:check type="radio" id="openGradCd" name="openGradCd" inputId="openGradCd" value="1" areaId="openGradContainer" titleId="cm.option.publ" checked="${wcSchdlBVo.openGradCd eq '1' ? 'true': 'false'}" onclick="chkOpenGradCd(this);"/>
						<c:if test="${param.fncCal ne 'dept' && wcSchdlBVo.schdlKndCd ne '3' && param.fncCal ne 'comp' && wcSchdlBVo.schdlKndCd ne '4'}">
							<m:check type="radio" id="openGradCd" name="openGradCd" inputId="openGradCd" value="2" areaId="openGradContainer" titleId="cm.option.apntPubl" checked="${wcSchdlBVo.openGradCd eq '2' ? 'true': 'false'}" onclick="chkOpenGradCd(this);"/>
						</c:if>
						<m:check type="radio" id="openGradCd" name="openGradCd" inputId="openGradCd" value="3" areaId="openGradContainer" titleId="cm.option.priv" checked="${empty wcSchdlBVo.openGradCd || wcSchdlBVo.openGradCd eq '3' ? 'true': 'false'}" onclick="chkOpenGradCd(this);"/>
						<c:if test="${param.fncCal eq 'dept' }"><m:check type="radio" id="openGradCd" name="openGradCd" inputId="openGradCd" value="4" areaId="openGradContainer" titleId="wc.cols.sel.dept" checked="${wcSchdlBVo.openGradCd eq '4' ? 'true': 'false'}" onclick="chkOpenGradCd(this);"/></c:if>
                    </dl>
                    </div>
                </dd>
 	...
 	<c:if test="${param.fncCal eq 'dept'}"><div class="entryzone">
		<div class="entryarea"><dl>
			<dd class="etr_bodytit">
			<div class="icotit_dot"><u:msg titleId="wc.cols.sel.dept" alt="부서선택" /></div>
			<div id="deptBtnArea" class="icoarea" style="left:40%;">
				<dl>
				<dd class="btn" onclick="addDept();" style="display:${!empty wcSchdlBVo.openGradCd && wcSchdlBVo.openGradCd eq '4' ? 'inline-block' : 'none'}"><u:msg titleId="cm.btn.add" alt="추가"/></dd>
				<dd class="btn" onclick="deptSelectDel();" style="display:${!empty wcSchdlBVo.openGradCd && wcSchdlBVo.openGradCd eq '4' ? 'inline-block' : 'none'}"><u:msg titleId="cm.btn.allDel" alt="전체삭제"/></dd>
				</dl>
			</div>
			</dd>
			<dd class="etr_input"><div class="etr_bodyline" id="deptSelectArea" style="min-height:40px;overflow-y:auto;"><c:forEach 
		...
		/></dd><dd class="btn" onclick="deptSelectDel(this);"></dd></dl></div>
		</c:forEach></div></dd>
			</dl>
		</div>
	</div>
	...
	<div class="entryzone">
		<div class="entryarea">
			...
			<dd class="etr_input"><div class="etr_bodyline editor" id="bodyHtmlArea" ><c:if test="${!empty wcSchdlBVo.cont }"><u:out value="${wcSchdlBVo.cont}" type="noscript" /></c:if></div></dd>
