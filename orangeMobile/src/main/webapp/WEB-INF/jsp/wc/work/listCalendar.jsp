...
function schdlEvent(actKey, val, sDate){
	var $form = $('#searchForm');
	var url = "/wc/work/listCalendar.do";
...
function viewSchdl(schdlId){
	...
	var url = "/wc/work/viewSchdl.do?menuId=${menuId}&schdlId="+schdlId+'&viewTyp=${param.viewTyp}&strtDt=${defaultDate}';
...
function viewSchdlPopTemp(obj){
	...
	var url = "/wc/work/viewSchdl.do?menuId=${menuId}&schdlId="+schdlId;
	$m.nav.next(event, url);
};
...
function loadEvents(start, end){
	...
	$m.ajax('/wc/work/listSchdlAjx.do?menuId=${menuId}', param, function(data) {
...
function setSchdl(schdlId){
	...
	//쓰기 권한 체크
	if('${writeAuth}' != 'Y') return;
	var url = "/wc/work/setSchdl.do?menuId=${menuId}&viewTyp=${viewTyp}";
	if(schdlId != null) {
		url+= "&schdlId="+schdlId;
	}
