...
function fnDelete(flag,schdlId){
	$m.msg.confirmMsg("cm.cfrm.del", null , function(result){
		if(result){
			...
			$m.ajax('/ct/schdl/'+transDelPage+'.do?menuId=${menuId}&ctId=${param.ctId}', {schdlId:schdlId, fncCal:'${fncCal }'}, function(data) {
				if (data.message != null) {
					$m.dialog.alert(data.message);
				}
				if (data.result == 'ok') {
					$m.nav.next(event, '/ct/schdl/${listPage}.do?menuId=${menuId}&ctId=${param.ctId}&${paramsForList }');
				}
			});
		}
	});
...
<% // 목록으로 이동 %>
function goList() {
	$m.nav.prev(event, '/ct/schdl/${listPage }.do?menuId=${menuId}&ctId=${param.ctId}&${paramsForList}');
}

<% // 등록 %>
function setSchdl(schdlId){
	var url = "/ct/schdl/setSchdl.do?menuId=${menuId}&ctId=${param.ctId}";
...
function viewAttchFile(id) {
	var url = "/ct/attachViewSub.do?menuId=${menuId}&ctId=${param.ctId}&schdlId=${CtSchdlBVo.schdlId}";
	url+='&fileIds='+id;
...
<section>
       ...
       <div class="btnarea" id="btnArea">
           ...
           <div class="size">
           <dl>
			       <u:secu auth="${promMod }"><dd class="btn" onclick="setSchdl('${CtSchdlBVo.schdlId }');"><u:msg titleId="cm.btn.mod" alt="수정" /></dd></u:secu>
			       <u:secu auth="${promDel }"><dd class="btn" onclick="fnDelete('del','${CtSchdlBVo.schdlId }');"><u:msg titleId="cm.btn.del" alt="삭제" /></dd></u:secu>
			       <c:if test="${scdPidCount > 1 && CtSchdlBVo.repetYn eq 'Y'}">
			       	   <u:secu auth="${promDel }"><dd class="btn" onclick="fnDelete('allDel','${CtSchdlBVo.schdlId }');"><u:msg titleId="cm.btn.allDel" alt="전체삭제" /></dd></u:secu>
			       </c:if>
       ...
       <!--titlezone S-->
        <div class="titlezone">
            <div class="titarea">
            <dl>
            <dd class="tit">${CtSchdlBVo.subj}</dd>
            <dd class="name">
            	<c:if test="${CtSchdlBVo.solaLunaYn == 'N' }">(<u:msg titleId="wc.option.luna" alt="음력"/>)</c:if>
	        	<c:choose>
		...
		<div id="tabViewArea">
			...
			<div class="bodyzone_scroll" id="cont">
				<div class="bodyarea">
					<dl>
						<dd class="bodytxt_scroll"><div class="scroll editor" id="bodyHtmlArea"><div id="zoom"><u:out value="${CtSchdlBVo.cont}" type="noscript" /></div></div>
						</dd>
					</dl>
				</div>
			</div>
			...
			<div  class="s_tablearea" id="detail" style="display:none;">
				...
				<table class="s_table">
					...
					<tr>
						...
						<td class="shead_lt"><u:out value="${CtSchdlBVo.schdlTypNm }" /></td>
					</tr>
					<tr>
						<th class="shead_lt"><u:msg titleId="cols.subj" alt="제목"/></th>
						<td class="shead_lt"><u:out value="${CtSchdlBVo.subj }" /></td>
					</tr>
					<tr>
						<th class="shead_lt"><u:msg titleId="cols.loc" alt="장소" /></th>
						<td class="shead_lt"><u:out value="${CtSchdlBVo.locNm }" /></td>
					</tr>
					<tr>
		...
		<div class="listarea" id="guest" style="display:none;"><!-- 참석자 목록 -->
			...
			<article id="apvLnUiArea">
				<c:choose>
					<c:when test="${!empty CtSchdlPromGuestDVoList }">
