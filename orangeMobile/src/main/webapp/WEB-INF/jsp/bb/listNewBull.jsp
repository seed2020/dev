...
function viewBull(brdId, id) {
	$m.nav.next(null,  '/bb/${viewPage}.do?${params}&brdId=' + brdId + '&bullId=' + id);
}
...
function readHst(id) {
	$m.dialog.open({
		...
		url:'/bb/listReadHstPop.do?menuId=${menuId}&bullId=' + id,
	});
}
//]]>
</script>
...
<div class="listarea">
	...
	<c:forEach items="${bbBullLVoList}" var="bbBullLVo" varStatus="status">
		<div class="listdiv" onclick="javascript:viewBull('${bbBullLVo.brdId}','${bbBullLVo.bullId}');">
	        ...
	        <dl>
              	<dd class="tit">
					...
					${bbBullLVo.subj}<c:if test="${bbBullLVo.cmtCnt > 0}">(<u:out value="${bbBullLVo.cmtCnt}" type="number" />)</c:if>
					<c:if test="${bbBullLVo.readYn != 'Y'}"></strong></c:if>
	        	</dd>
