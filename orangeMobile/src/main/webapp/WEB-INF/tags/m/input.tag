...
if("view".equals(type)){
	...
	buffer.append(" class=\"");
	if(className == null || className.isEmpty()) className = "etr_iplt";
...
<script type="text/javascript">
	...
	%>
		if(eventNm == null && isEmptyVa(va)<%if(mandatorySkipper!=null){%> && !skipMandatoryVa<%}%>){
			$m.msg.alertMsg('cm.input.check.mandatory',['<%=title%>']);
			return false;
		}<%
	}
	...
	%>
		...
		if(ck!=0){
			if(ck>0) {
				$m.msg.alertMsg('cm.input.check.maxbyte',['<%=title%>','<%=maxByte%>']);
				$('#<%=id%>').val(cutUtf8(va, '<%=maxByte%>'));
			}<%
			if(!"null".equals(minByte)){%>
			if(eventNm == null && ck<0) {
				$m.msg.alertMsg('cm.input.check.minbyte',['<%=title%>','<%=minByte%>']);
			}<%}
	...
	%>
		if(eventNm == null && va!='' && va.length < <%= minLength%>){
			$m.msg.alertMsg('cm.input.check.minlength',['<%=title%>','<%=minLength%>']);
			return false;
		}
	<%
	...
	%>
		...
		if(ck!=0){
			if(ck>0) {
				$m.msg.alertMsg('cm.input.check.maxint',['<%=title%>','<%=maxInt%>']);
				$('#<%=id%>').val('<%=maxInt%>');
			}<%
			if(!"null".equals(minInt)){%>
			if(eventNm == null && ck<0) {
				$m.msg.alertMsg('cm.input.check.minint',['<%=title%>','<%=minInt%>']);
				$('#<%=id%>').val('<%=minInt%>');
			}<%}
	...
	%>
		...
		if(ck>0){
			if(ck>0) {
				$m.msg.alertMsg('cm.input.check.maxfloat',['<%=title%>','<%=maxFloat%>']);
				$('#<%=id%>').val('<%=maxFloat%>');
			}<%
			if(!"null".equals(minFloat)){%>
			if(eventNm == null && ck<0) {
				$m.msg.alertMsg('cm.input.check.minfloat',['<%=title%>','<%=minFloat%>']);
				$('#<%=id%>').val('<%=minFloat%>');
			}<%}
