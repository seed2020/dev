...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		this._compareValues=function(func,f,v,how,t){
			var fld;
			if(_useProperties){
				fld='ce$.jgrid.getAccessor(this,\''+f+'\')';
			}else{
				fld='this';
			}
			...
			switch(swst) {
				...
				case 'datetime':
					...
					fld = 'ce$.jgrid.parseDate("'+t.srcfmt+'",'+fld+').getTime()';
					break;
				default :
		...
		this.startsWith=function(f,v){
			...
			if(_useProperties){
				self._append(self._getStr('ce$.jgrid.getAccessor(this,\''+f+'\')')+'.substr(0,'+length+') == '+self._getStr('"'+self._toStr(v)+'"'));
			}else{
		...
		this.endsWith=function(f,v){
			...
			length=_trim ? $.trim(val.toString()).length:val.toString().length;
			if(_useProperties){
				self._append(self._getStr('ce$.jgrid.getAccessor(this,\''+f+'\')')+'.substr('+self._getStr('ce$.jgrid.getAccessor(this,\''+f+'\')')+'.length-'+length+','+length+') == "'+self._toStr(v)+'"');
			} else {
		...
		this.contains=function(f,v){
			if(_useProperties){
				self._append(self._getStr('ce$.jgrid.getAccessor(this,\''+f+'\')')+'.indexOf("'+self._toStr(v)+'",0) > -1');
			}else{
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		getOffset = function (iCol) {
			...
			var actualLeft = 0;
			var visibleCols = 0;
			$(ts.grid.hDiv).find('th').each(function(index, el) {
				if(index <= iCol) {
					if(ts.p.colModel[index].hidden === false ) {
						actualLeft += $(el).outerWidth();
						visibleCols ++;
					}
				}
			});
			...
			// Webkit 계열 브라우저의 110% zoom 이 비정상 동작하여 보정
			var zoomLevel = parseInt((window.outerWidth - 8) / window.innerWidth * 100);
			if(zoomLevel > 107 && zoomLevel < 115) {
				ret[0] = actualLeft - visibleCols;
			}
		...
		$("th",thr).each(function ( j ) {
			...
			if(ts.p.colModel[j].resizable){
				...
				if($.browser && !$.browser.opera) { $(res).css("cursor","col-resize"); }
...
var ie6;
if($.browser){ie6 = $.browser.msie&&($.browser.version == "6.0");}
else {ie6 = false;}
var F=false,
...
f1=function(k){return parseInt(E1.css(k),10)||false;};
	...
	Author:      Michael Sch첩ler/2008-01-29
...
"use strict";
	...
	$.fn.fmatter.actions = function(cellval,opts) {
		...
		if(op.editformbutton){
			ocl = "onclick=ce$.fn.fmatter.rowactions.call(this,'formedit'); onmouseover=ce$(this).addClass('ui-state-hover'); onmouseout=ce$(this).removeClass('ui-state-hover'); ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		} else if(op.editbutton){
			ocl = "onclick=ce$.fn.fmatter.rowactions.call(this,'edit'); onmouseover=ce$(this).addClass('ui-state-hover'); onmouseout=ce$(this).removeClass('ui-state-hover') ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		}
		if(op.delbutton) {
			ocl = "onclick=ce$.fn.fmatter.rowactions.call(this,'del'); onmouseover=ce$(this).addClass('ui-state-hover'); onmouseout=ce$(this).removeClass('ui-state-hover'); ";
			str += "<div title='"+$.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+ocl+"><span class='ui-icon ui-icon-trash'></span></div>";
		}
		ocl = "onclick=ce$.fn.fmatter.rowactions.call(this,'save'); onmouseover=ce$(this).addClass('ui-state-hover'); onmouseout=ce$(this).removeClass('ui-state-hover'); ";
		str += "<div title='"+$.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save' "+ocl+"><span class='ui-icon ui-icon-disk'></span></div>";
		ocl = "onclick=ce$.fn.fmatter.rowactions.call(this,'cancel'); onmouseover=ce$(this).addClass('ui-state-hover'); onmouseout=ce$(this).removeClass('ui-state-hover'); ";
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				var agentInfo = parent.agentInfo || {};
				if(iRow == 1 || ($('#gridList').hasClass('data-multirow') && iCol == 1)) {
					elc.maxLength = 20;
					if(agentInfo.IsIE10){
						$(elc).off("keyup").on("keyup", function(e){
							if(this.value.length > elc.maxLength){
								this.value = this.value.substring(0, elc.maxLength);
							}
						});
					}else{
						$(elc).off("keyup").on("keyup", function(e){
					...
					if(agentInfo.IsGecko){
						$(elc).css('imeMode','disabled').keypress(function(event){
							$($t).jqGrid("checkInteger",event,  $(this), 'chartGrid');
	...
	checkInteger : function (e, ele, type) { //type :'minus', 'integer', 'point'
		var isDefaultKey = function( key ){
			var passedBaseKey = ["arrowleft", "arrowright", "arrowup", "arrowdown", "backspace", "delete", "tab", "space", "numlock", "ctrl", "shift", "alt", "enter"];
			return ( $.inArray(key, passedBaseKey) > -1 );
		}

		var regTest = function(reg, key){
			return reg.test(key);
		}

		var valueEdit = function(reg, str, condition){
			if(condition == undefined || condition){
				ele.val( ele.val().replace(reg, str) );
			}
		}

		if(e != null){

			if(typeof e.key === 'undefined' && e.keyCode) {
				...
				switch(e.keyCode) {
					...
					case 56:
					case 57:
						e.key = '0123456789'.charAt(e.keyCode-48);
						break;
					//arrow key
					case 37:
					case 38:
					case 39:
					case 40:
						e.key = 'Arrow' + (['Left', 'Up', 'Right','Down'])[e.keyCode-37];
						break;
					//case
			...
			var agentInfo = parent.agentInfo || {};
			if(agentInfo.IsIE || agentInfo.IsIE10 || agentInfo.IsIE11 || agentInfo.IsEdge) {
				if(e.keyCode === 46 && char === '.') {
					e.key = '.';
				}
			}
			if((e.key === 'Delete' || e.key === 'Del' || e.key === 'Decimal') && char ==='.') {
				e.key = '.';
			}
			...
			if( !isDefaultKey(key) && !e.ctrlKey ) {
				var eventCancel = false;
				if(type == 'integer' && !regTest( intReg, key ) ){ // 크롬에서 한글 키입력일 경우 숫자 정규식이 작동 안됨.
					eventCancel = true;
					valueEdit(/[^\d]/gi, '', e.type == 'keyup');
				} else if (type == 'minus') {
					if(!regTest( int_minusReg, key ))
						eventCancel = true;

					if(key == '-'){ // 어느 곳에서 minus를 입력해도 맨 처음 위치로 이동.
						eventCancel = true;
						ele.val( key + ele.val().replace(/[-]/gi, "") );
					}
					valueEdit(/[^\d-]/gi, '', e.type == 'keyup');
				} else if (type == 'point') {
					// 숫자, - 키 입력 이외 OR 이미 소수 점이 하나 있을 때
					if( !regTest( int_pointReg, key ) || (key == '.' && ele.val().split(".").length >= 2))
						eventCancel = true;
					valueEdit(/[^\d.]/gi, '', e.type == 'keyup');
				} else if (type == 'chartGrid') {
					// 숫자, -, ., , 키 입력 이외 OR 이미 소수 점이 하나 있을 때
					if( !regTest( int_chartReg, key ) || (key == '.' && ele.val().split(".").length >= 2))
						eventCancel = true;
					valueEdit(/[^\d.,]/gi, '', e.type == 'keyup');
				}
				...
				if(eventCancel){
					...
					valueEdit(/[a-z|ㄱ-ㅎ|ㅏ-ㅣ]/gi, '');
				}
			}
		} else {
			var checkEle = false;
			if(ele.hasClass('chartMaxValue')||ele.hasClass('chartMinValue')||ele.hasClass('chartIncrement')||ele.hasClass('colCount')||ele.hasClass('rowCount')){
				checkEle = true;
			}
			...
			if(checkEle) {
				...
				if(tempVal != null && tempVal != ""){
					...
					if(type == 'minus') {//마이너스(-) 허용
						if(tempVal.charAt(0) == '-') {
							startPoint = 1;
							tempVal = tempVal.substring(startPoint);
						}
						tempVal = tempVal.replace(/[^0-9]/g,'');
						if(startPoint == 1) {
							tempVal = '-' + tempVal;
						}
					} else if(type == 'point') {//소수점(.) 허용
						tempVal = tempVal.replace(/[^0-9.]/g,'');
					} else if(type == 'integer') {//숫자만 허용
						tempVal = tempVal.replace(/[^0-9]/g,'');
					} else if(type == 'chartGrid') {//소수점(.) 허용 && 방향키
						tempVal = tempVal.replace(/[^0-9.,]/g,'');
					}
				}

				if(tempVal != null) ele.val(tempVal);
			}
		}
	},
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				hid = clid+"_"+i;
				icon = "<span style='cursor:pointer;' class='ui-icon "+pmrtl+"' onclick=\"ce$('#"+$.jgrid.jqID($t.p.id)+"').jqGrid('groupingToggle','"+hid+"');return false;\"></span>";
				try {
	...
	sortableColumns : function (tblrow)
	{
		return this.each(function (){
			...
			var sortable_opts = {
				...
				/*** header column 포커스 상태에서 드래그시 다른 필드 데이터 사라지는현상***/
	...
	sortableRows : function (opts) {
		...
		return this.each(function(){
			...
			if($.fn.sortable) {
				...
				$tbody.on('mouseenter','td input', function() {
					$tbody.enableSelection();
				}).on('mouseleave', 'td input', function() {
...
ce$(selector).each(function() {
	...
	var inputCheckbox = ce$('tr td:first-child input[type=checkbox]:first', ce$(this));
	var inputRadio = ce$('tr td:first-child input[type=radio]:first', ce$(this));
	...
	ce$('th', ce$(this)).each(function() {
		if (colModel.length === 0 && selectable) {
			...
			colModel.push({
				name: ce$(this).attr("id") || ce$.trim(ce$.jgrid.stripHtml(ce$(this).html())).split(' ').join('_'),
				index: ce$(this).attr("id") || ce$.trim(ce$.jgrid.stripHtml(ce$(this).html())).split(' ').join('_'),
				width: ce$(this).width() || 150
			});
			colNames.push(ce$(this).html());
		}
	});
	var data = [];
	var rowIds = [];
	var rowChecked = [];
	ce$('tbody > tr', ce$(this)).each(function() {
		var row = {};
		var rowPos = 0;
		ce$('td', ce$(this)).each(function() {
			if (rowPos === 0 && selectable) {
				var input = ce$('input', ce$(this));
				...
				row[colModel[rowPos].name] = ce$(this).html();
			}
			rowPos++;
		});
	...
	// Mark it as jqGrid
	ce$(this).addClass("scroll");

	ce$(this).jqGrid(ce$.extend({
		datatype: "local",
		width: w,
		colNames: colNames,
		colModel: colModel,
	...
	for (a = 0; a < data.length; a++) {
		...
		ce$(this).jqGrid("addRowData",id, data[a]);
	}

	// Set the selection
	for (a = 0; a < rowChecked.length; a++) {
		ce$(this).jqGrid("setSelection",rowChecked[a]);
	}
});
};
