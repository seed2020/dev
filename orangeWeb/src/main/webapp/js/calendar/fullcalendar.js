...
var browser = { ie:false, chrome:false, firefox:false, safari:false, opera:false, etc:false, ver:0, ieCompatibility:false,
	init:function(){
...
// 소스수정 : if($('#calendar').fullCalendar('getView').name=='agendaWeek' && unit=='month') return false;
function formatSimilarChunk(date1, date2, unzonedDate1, unzonedDate2, chunk) {
	...
	if (typeof chunk === 'string') { // a literal string
		...
		if (unit && unzonedDate1.isSame(unzonedDate2, unit)) {
			if($('#calendar').fullCalendar('getView').name=='agendaWeek' && unit=='month') return false;
...
var DayTableMixin = FC.DayTableMixin = {
	...
	// 소스수정 : return this.view.opt('dayOfMonthFormat');
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var titleHtml;
		// 소스수정
		var allowDisplay = view.opt('allowDisplay');
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				...
				// 소스수정
					(allowDisplay && !seg.isStart ? '<span class="fc-more-left"></span>' : '') +
					(this.isRTL ?
						titleHtml + ' ' + timeHtml : // put a natural space in between
						timeHtml + ' ' + titleHtml   //
						) +
					(allowDisplay && !seg.isEnd ? '<span class="fc-more-right"></span>' : '') +
				'</div>' +
	...
	renderSegRow: function(row, rowSegs) {
		...
		// 수정 - ie 호환성보기에서 ui 틀어지는 문제 해결 
		var loneCellRow = []; // rowspan array - ie7
		// brower check
		var isIe=browser.ie && browser.ver==7;
		...
		function emptyCellsUntil(endCol) {
			while (col < endCol) {
				...
				rowspan=1;
				if (td) {
					if(isIe) rowspan=(loneCellRow[i - 1] || [])[col]+1;
					else rowspan=parseInt(td.attr('rowspan') || 1, 10) + 1;
					td.attr(
						'rowspan',
						rowspan
					);
					...
					td = $('<td/>');
					tr.append(td);
					if(isIe) loneCellRow[i][col] = rowspan;
				}
				cellMatrix[i][col] = td;
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					...
					(event.title ?
						'<div class="fc-title"' +
						(timeText ? ' data-start="' + htmlEscape(startTimeText) + '"' : '') +
						'>' +
...
var momComputableOptions = {
	...
	// Produces format strings like "ddd M/D" -> "Fri 9/15"
	// 소스수정 : dayOfDayFormat 
	dayOfDayFormat: function(momOptions, fcOptions) {
		var format = momOptions.longDateFormat('l'); // for the format like "M/D/YYYY"
		...
		if (fcOptions.isRTL) {
			format += ' dddd'; // for RTL, add day-of-week to end
		}
		else {
			format = 'dddd ' + format; // for LTR, add day-of-week to beginning
		}
		return format;
	},
				...
				'<tbody class="fc-body">' +
					'<tr>' +
						'<td class="' + this.widgetContentClass + '">' +
							(this.dayGrid ?
								'<div class="fc-day-grid"/>' +
								'<div class="fc-divider ' + this.widgetHeaderClass + '"/>' :
								''
								) +
