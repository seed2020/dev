...
icon	: 버튼내의 이미지
	ico_delete.png, ico_add.png, ico_minus.png, ico_top.png, ico_bottom.png, ico_print.png
*/
...
if(SecuUtil.hasAuth(request, auth, ownerUid)){
	...
	if(titleButtonType==null){
		out.println("\r\n			<li class=\"btn\">");	// li btn 테그 - open
		request.setAttribute("titleSubType", "btn");
	} else if(!"btn".equals(titleButtonType)){
...
%><a<%=id%> href="<%=href%>"<%=onclick %> class="sbutton button_small"<%=style%> title="<%= koDesc%>"><span><%
if(img!=null){
	%><img src="${_cxPth}/images/${_skin}/<%= img%>" width="11" height="9" alt="<%= title%>" /><%
}
%><%= title%></span></a><%
}
%>