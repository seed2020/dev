...
if(SecuUtil.hasAuth(request, auth, ownerUid)){
	...
	if(tabSubType==null || "tab".equals(tabSubType)){
		...
		out.println("\r\n		<li class=\"btn\">");	// li 테그 for btn - open
		request.setAttribute("tabSubType", "btn");
	} else if(!"btn".equals(tabSubType)){
	...
	if(img!=null){
		...
		img = "<img src=\""+request.getAttribute("_cxPth")+"/images/"+request.getAttribute("_skin")+"/"+img+"\" width=\"" + imgW + "\" height=\"" + imgH + "\" alt=\""+title+"\" />";
	} else {
		img = "";
	}

	style = (style==null) ? "" : " style=\""+style+"\"";
	spanStyle = (spanStyle==null) ? "" : " style=\""+spanStyle+"\"";
%><a<%=id%> href="<%=href%>"<%=onclick %> class="sbutton button_small"<%=style%> title="<%= koDesc%>"><span<%=spanStyle%>><%= img%><%= title%></span></a><%
}
%>