...
<!--
//마감여부 저장
function endModSave(survId, ctId){
	if(true/*confirmMsg("cm.cfrm.save")*/ ){
		var $form = $("#endModForm");
		$form.attr("method", "POST");
		$form.attr("action", "./transEndModSave.do?menuId=${menuId}&survId="+survId+"&ctId=" +ctId);
		$form.submit();
	}
}
...
function startDtNotMod(){
	alert('<u:msg titleId="wv.msg.set.notChgStrtDt" alt="시작일은 변경할 수 없습니다."/>');
	return false;
}
...
<u:listArea>
	...
	<td class="bodybg_lt"><table border="0" cellpadding="0" cellspacing="0">
		...
		<td class="width5"></td>
		<td ><u:calendar id="strtDt" option="{end:'finDt'}"  handler="function(date,option){return startDtNotMod();}" titleId="cols.choiDt" alt="선택일시" readonly="true" value="${ctsVo.survStartDt}" /></td>
		</tr>
		<tr>
...
<u:buttonArea>
	<u:button titleId="cm.btn.save" onclick="javascript:endModSave('${ctsVo.survId}','${ctsVo.ctId}');" alt="저장"/>
