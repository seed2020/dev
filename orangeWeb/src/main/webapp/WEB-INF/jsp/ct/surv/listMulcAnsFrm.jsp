...
function applyMulcAnsTooltip() {
	$('.ct_survMulcAnsInfo').mouseover(function(event){
	...
	$('.ct_survMulcAnsInfo').mouseout(function(event){
...
<div style="width:580px; padding:0px 0px 0px 3px" >
	<form id="listDeptStatForm" action="./listMulcAnsFrm.do">
		...
		<div class="titlearea">
			...
			<dl>
				<dd class="title_s">
					<div class="ellipsis" title="<u:msg titleId="cols.ques" alt="질문" />${ctsQueVo.quesSortOrdr} ) ${ctsQueVo.quesCont}">
						<input type="hidden" id="quesReplyNm" name="quesReplyNm" value="<u:msg titleId="cols.ques" alt="질문" />${ctsQueVo.quesSortOrdr} ) ${ctsQueVo.quesCont}"/>
						<u:msg titleId="cols.ques" alt="질문" /><u:out value="${ctsQueVo.quesSortOrdr} ) ${ctsQueVo.quesCont}" maxLength="65" />
					</div>
				</dd>
			</dl>
			</div>
		...
		<div class="listarea">
			<table class="listtable" border="0" cellpadding="0" cellspacing="1" style="width:100%;table-layout:fixed;">
				...
				<c:forEach  var="ctsMulc" items="${ctSurvBMapList}"  varStatus="status">
					<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
						<td class="body_ct"><u:out value="${ctsMulc.rnum}"/></td>
						
						<td class="body_lt">
							<div class="ellipsis" title="${ctsMulc.mulcInputReplyCont}">
								<u:out value="${ctsMulc.mulcInputReplyCont}"/>
							</div>
						</td>
					</tr>
				</c:forEach>
