...
function setBumkSelect(){
	...
	if(schdlKndCd != ''){
		...
		$.ajax({
	        url: './selectBumkAjx.do?menuId=${menuId}',
	        type: 'POST',
	        data:{
...
function showAll() {
	...
	if($('#submitTab').val() == 'molyList'){
		...
		$form.attr('action','./listNewSchdl.do?menuId=${menuId}&ctId=${ctId}');
...
function showSchdlList(val){
	...
	if($('#submitTab').val() == 'molyList'){
		...
		$form.attr('method','post');
		$form.attr('action','./listNewSchdl.do?menuId=${menuId}&ctId=${ctId}&catId='+val);
...
function openSingUser(){
	...
	$form.attr('method','post');
	$form.attr('action','./listNewSchdl.do?${params}');
	$('#tabNo').val('0');
	$form.submit();
	}else if($('#submitTab').val() == 'welyList'){
	$form.attr('method','post');
...
function bumkSelect(obj){
	if(obj.value=='-1'){
		location.href="./listNewSchdl.do?${params}";
		return;
	}
...
<tr>
	...
	<td class="${calDay.toDayFlag}">
		<span class="${calDay.holiFlag}" 
			<c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${calDay.year}','${calDay.month}','${calDay.day}');" 
			style="cursor:pointer;"
			</c:if>
		><u:out value="${calDay.day}"/></span>
		...
		<c:otherwise>
			...
			<div ${catBgCss } class="more_${moreParam }_month${!empty scds.subj ? '' : 'none'} ellipsis">
				...
				<span class="scdArea scd_${scds.schdlTypCd } ${scds_color eq 'scd_red' ? '' : scds_color}" id="${scds.schdlId}_month" ${catFontCss }>
...
<div id="welyList" class="listarea" style="display: none;">
	...
	<tr>
		...
		<td width="15%" class="${day_num_style}" 
			<c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${weekCalDay.year}','${weekCalDay.month}','${weekCalDay.day}');" 
			style="cursor:pointer;"
			</c:if>
		><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.sun" alt="일" /></td>
		</c:when>
		<c:when test="${status.index==1}">
		<td width="14%" class="${day_num_style}" 
		<c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${weekCalDay.year}','${weekCalDay.month}','${weekCalDay.day}');" 
			style="cursor:pointer;"
			</c:if>
		><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.mon" alt="월" /></td>
		</c:when>
		...
		><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.tue" alt="화" /></td>
		</c:when>
		<c:when test="${status.index==3}">
		<td width="14%" class="${day_num_style}" <c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${weekCalDay.year}','${weekCalDay.month}','${weekCalDay.day}');" 
			style="cursor:pointer;"
			</c:if>><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.wed" alt="수" /></td>
		</c:when>
		<c:when test="${status.index==4}">
		<td width="14%" class="${day_num_style}" <c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${weekCalDay.year}','${weekCalDay.month}','${weekCalDay.day}');" 
			style="cursor:pointer;"
			</c:if>><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.thu" alt="목" /></td>
		</c:when>
		<c:when test="${status.index==5}">
		<td width="14%" class="${day_num_style}" <c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${weekCalDay.year}','${weekCalDay.month}','${weekCalDay.day}');" 
			style="cursor:pointer;"
			</c:if>><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.fri" alt="금" /></td>
		</c:when>
		<c:when test="${status.index==6}">
		<td width="15%" class="${day_num_style}" <c:if test="${!empty authChkW && authChkW == 'W' }">
			onclick="setSchdlPop('${weekCalDay.year}','${weekCalDay.month}','${weekCalDay.day}');" 
			style="cursor:pointer;"
			</c:if>><strong><u:out value="${weekCalDay.day}"/></strong> <u:msg titleId="wc.cols.sat" alt="토" /></td>
		</c:when>
		</c:choose>
		</c:forEach>
	</tr>
		...
		<div id="dalyList" class="listarea" style="display: none;" noBottomBlank="true">
			<table class="listtable" border="0" cellpadding="0" cellspacing="1">
				...
				<tr>
					...
					<td class="head_lt" <c:if test="${!empty authChkW && authChkW == 'W' }">
						onclick="setSchdlPop('${wcScdCalDay.year}','${wcScdCalDay.month}','${wcScdCalDay.day}');" 
						style="cursor:pointer;"
						</c:if>>
			...
			<table class="listtable" border="0" cellpadding="0" cellspacing="1">
				...
				<tr>
					<td width="70" class="scdtd_day" <c:if test="${!empty authChkW && authChkW == 'W' }">
						onclick="setSchdlPop('${wcScdCalDay.year}','${wcScdCalDay.month}','${wcScdCalDay.day}','${tempHour }:00');" style="cursor:pointer;"
					</c:if>>
					...
					<div id="tooltipContainer" class="tooltipCls" style="position:absolute;display:none;z-index:999;margin-top:14px;">
						<div id="tooltip" class="tooltip" style="position:absolute;display:block;z-index:999;">
							...
							<span class="scdArea scd_${scds.schdlTypCd } ${scds_color}"  id="${scds.schdlId}_day" ${catFontCss }>${convStartTime} 
								<u:out value="${scds.subj}" maxLength="15" />
							</span>
