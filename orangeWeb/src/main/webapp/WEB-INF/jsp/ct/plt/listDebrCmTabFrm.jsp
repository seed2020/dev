...
function viewCm(ctId, fncUid, debrId) {
	//location.href = "./leftCt.do?menuId="+ fncUid +"&ctId=" + ctId + "&prevMenuId=${menuId}";
	top.location.href = "/ct/debr/listOpin.do?menuId="+ fncUid +"&ctId=" + ctId + "&debrId="+debrId;
}
				...
				<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
					<c:forTokens	items="${bodyList}" var="colId" varStatus="colStatus" delims=",">
						<td class="body_${colId }" id="list_${colStatus.count == 1 ? colId : '' }" style="display:none;">
							<c:choose>
								<c:when test="${colId eq 'ctNm' }"><div class="ellipsis" title="<u:convertMap srcId="mapList" attId="${colId}" type="html" /><u:out value="${mapList.subj}"/>"><a href="javascript:viewCm('${mapList.ctId}','${mapList.ctFncUid}','${mapList.debrId}');">[<u:convertMap srcId="mapList" attId="${colId}" type="html" />]<u:out value="${mapList.subj}"/></a></div></c:when>
