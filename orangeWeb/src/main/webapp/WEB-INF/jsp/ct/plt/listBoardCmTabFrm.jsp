...
function viewCm(ctId, fncUid,bullId) {
	top.location.href = "/ct/board/viewBoard.do?menuId="+ fncUid +"&ctId=" + ctId +"&bullId=" + bullId;
}
				...
				<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
					<c:forTokens	items="${bodyList}" var="colId" varStatus="colStatus" delims=",">
						<td class="body_${colId }" id="list_${colStatus.count == 1 ? colId : '' }" style="display:none;">
							<c:choose>
								<c:when test="${colId eq 'ctNm' }"><div class="ellipsis" title="<u:convertMap srcId="mapList" attId="${colId}" type="html" />"><a href="javascript:viewCm('${mapList.ctId}','${mapList.ctFncUid}','${mapList.bullId}');">[<u:convertMap srcId="mapList" attId="${colId}" type="html" />]${mapList.subj}</a></div></c:when>
