...
function saveBbPltSetup(){
	...
	if(arr.length>5){<%
		...
		var result = false;
		callAjax("${_cxPth}/ct/plt/transNoPrListBxPltSetupAjx.do?menuId=${menuId}", {pltId:'${param.pltId}', bxIds:arr.join(',')}, function(data){
...
<c:forEach items="${CtPltNoPrListDVoList}" var="CtPltNoPrListDVo" varStatus="status">
	<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
		<td class="bodybg_ct"><input type="checkbox" name="ctPltSetupCheck" id="${CtPltNoPrListDVo.bxId}BbPltSetup"
			value="${CtPltNoPrListDVo.bxId}" ${CtPltNoPrListDVo.useYn=='Y' ? 'checked="checked"' : ''} /></td>
		<td class="body_lt"><label for="${CtPltNoPrListDVo.bxId}BbPltSetup"><u:titleText title="${CtPltNoPrListDVo.bxNm }" noLi="true"/></label></td>
	</tr>
</c:forEach>
</u:listArea>
