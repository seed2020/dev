...
function saveBbPltSetup(){
	...
	if(arr.length>5){<%
		...
		var result = false;
		callAjax("${_cxPth}/ct/plt/transCommunityListBxPltSetupAjx.do?menuId=${menuId}", {pltId:'${param.pltId}', bxIds:arr.join(',')}, function(data){
...
<c:forEach items="${ctPltCmListDVoList}" var="ctPltCmListDVo" varStatus="status">
	<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
		<td class="bodybg_ct"><input type="checkbox" name="ctPltSetupCheck" id="${ctPltCmListDVo.bxId}BbPltSetup"
			value="${ctPltCmListDVo.bxId}" ${ctPltCmListDVo.useYn=='Y' ? 'checked="checked"' : ''} /></td>
		<td class="body_lt"><label for="${ctPltCmListDVo.bxId}BbPltSetup"><u:titleText title="${ctPltCmListDVo.bxNm }" noLi="true"/></label></td>
	</tr>
</c:forEach>
</u:listArea>
