		...
		<c:when test="${!empty rsltMapList}">
			<c:forEach var="mapList" items="${rsltMapList}" varStatus="status">
				...
				<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
					<c:forTokens	items="${bodyList}" var="colId" varStatus="colStatus" delims=",">
						<td class="body_${colId }" id="list_${colStatus.count < 3 ? colId : '' }" style="display:none;">
							<c:choose>
								<c:when test="${colId eq 'ctNm' }"><div class="ellipsis" title="<u:convertMap srcId="mapList" attId="${colId}" type="html" />"><a href="javascript:viewCm('${mapList.ctId}','${mapList.ctFncUid}');"><u:convertMap srcId="mapList" attId="${colId}" type="html" />(${mapList.mbshCnt})</a></div></c:when>
