...
function getCheckedTrs(noSelectMsg){
	var arr=[], id, obj;
	$("#catListArea tbody:first input[type='checkbox']:checked").each(function(){
...
<%// 다음 Row 번호 %>
var gMaxRow = parseInt('${fn:length(whCatGrpLVoList)}');
<%// 행추가%>
function addCatRow(){
...
<%// 선택삭제%>
function delCatSelRow(){
	var arr = getCheckedTrs("cm.msg.noSelect");
	if(arr!=null) delCatRowInArr(arr);
}
<%// 삭제 - 배열에 담긴 목록%>
function delCatRowInArr(rowArr){
	var delVa = $("#delList").val(), delArr = [], $mdId;
	if(delVa!='') delArr.push(delVa);
	for(var i=0;i<rowArr.length;i++){
		$mdId = $(rowArr[i]).find("input[name='catNo']");
		if($mdId.val()!=''){
...
function saveCat() {
	if (validator.validate('grpArea')) {
		...
		var arr = releaseDisable($('#catListArea select'));
		
		var $form = $('#setForm');
		$form.attr('method','post');
		$form.attr('action','./transCatGrpList.do?menuId=${menuId}');
...
<c:if test="${!empty whCatGrpBVo }">
<u:input type="hidden" id="catGrpId" value="${whCatGrpBVo.catGrpId }"/>
</c:if>
<u:title titleId="wh.jsp.typGrp.title" type="small" alt="유형그룹" />
<% // 폼 필드 %>
	<u:listArea id="grpArea" colgroup="20%,">
	<tr>
				...
				<tr>
					<td id="langTypArea">
						<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="status">
							<u:convert srcId="${whCatGrpBVo.rescId}_${langTypCdVo.cd}" var="rescVa" />
						...
						<u:input type="hidden" id="grpRescId" value="${whCatGrpBVo.rescId}" />
					</td>
	...
	<tr>
		...
		<td>
			<u:checkArea>
				<u:radio name="useYn" value="Y" titleId="cm.option.use" alt="사용" checkValue="${whCatGrpBVo.useYn }"  inputClass="bodybg_lt" checked="${empty whCatGrpBVo.useYn }"/>
				<u:radio name="useYn" value="N" titleId="cm.option.notUse" alt="사용안함" checkValue="${whCatGrpBVo.useYn }" inputClass="bodybg_lt" />
			</u:checkArea>
		</td>
	</tr>
	<tr>
		...
		<td>
			<u:checkArea>
				<u:radio name="dftYn" value="Y" titleId="cm.option.use" alt="사용" checkValue="${whCatGrpBVo.dftYn }"  inputClass="bodybg_lt" />
				<u:radio name="dftYn" value="N" titleId="cm.option.notUse" alt="사용안함" checkValue="${whCatGrpBVo.dftYn }" inputClass="bodybg_lt" checked="${empty whCatGrpBVo.dftYn }"/>
			</u:checkArea>
		</td>
	</tr>
</u:listArea>
<u:blank />
<u:title titleId="wh.jsp.typList.title" type="small" alt="유형목록" >
<u:titleButton href="javascript:addCatRow();" id="btnAddRow" titleId="cm.btn.plus" alt="행추가" auth="W" />
<u:titleButton href="javascript:delCatRow();" id="btnDelRow" titleId="cm.btn.minus" alt="행삭제" auth="W" />
<u:titleButton href="javascript:delCatSelRow();" id="btnDelSel" titleId="cm.btn.selDel" alt="선택삭제" auth="W" />
		
<u:titleIcon type="up" onclick="moveCat('up');" auth="W" />
<u:titleIcon type="down" onclick="moveCat('down');" auth="W" />
</u:title>
<u:listArea id="catListArea" style="height:220px;overflow-y:auto;" colgroup="15%,">	
	<tr id="headerTr">
		<th class="head_ct"><input type="checkbox" id="checkHeader" title="<u:msg titleId='cm.check.all' />" onclick="checkAllCheckbox('catListArea', this.checked);" value=""/></th>
		<th class="head_ct"><u:mandatory /><a href="javascript:changeLangTypCds('catListArea');" title="<u:msg titleId="pt.jsp.terms.chgLangAll" alt="일괄 언어 변경" />"><u:msg titleId="wh.cols.hdl.typ" alt="처리유형"/></a></th>
	</tr>
	<u:set var="nodataStyle" test="${fn:length(whCatGrpLVoList)>1}" value="style=\"display:none;\"" elseValue=""/>
	<tr id="nodata" ${nodataStyle }>
	<td class="nodata" colspan="2"><u:msg titleId="cm.msg.noData" alt="해당하는 데이터가 없습니다." /></td>
	</tr>
	<c:if test="${fn:length(whCatGrpLVoList)>0}">
		<c:forEach items="${whCatGrpLVoList}" var="vo" varStatus="status"
			 ><u:set test="${status.last}" var="trDisp" value="display:none"
			/><u:set test="${status.last}" var="trId" value="hiddenTr" elseValue="tr${vo.catNo}"
			...
			<tr onmouseover="this.className='trover'" onmouseout="this.className='trout'" id="${trId}" style="${trDisp}" >
				<td class="bodybg_ct"><input type="checkbox" id="listChk" value="${vo.catNo }" /><u:input type="hidden" id="catNo" value="${vo.catNo }"/></td>
		...
		<u:set test="${fn:length(_langTypCdListByCompId)==1}" var="titlePrefix" value="" elseValue="${langTypCdVo.rescNm}" />
		<u:input id="rescVa_${langTypCdVo.cd}${lstIndex}" name="rescVa_${langTypCdVo.cd}" titlePrefix="${titlePrefix}" titleId="wh.cols.hdl.typNm" value="${rescVa}" style="${style}"
			maxByte="200" validator="changeLangSelectorCat('catListArea', id, va)" mandatory="Y" />
		</c:forEach>
		</td>
		<td>
		<c:if test="${fn:length(_langTypCdListByCompId)>1}">
			<select id="langSelector${lstIndex}" onchange="changeLangTypCd('catListArea','langTyp${lstIndex}', this.value)" <u:elemTitle titleId="cols.langTyp" />>
			<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="langStatus">
			<u:option value="${langTypCdVo.cd}" title="${langTypCdVo.rescNm}" />
			</c:forEach>
			</select>
...
<u:buttonArea style="clear:both;">
	<u:button titleId="cm.btn.save" onclick="saveCat();" alt="저장" auth="W" />
