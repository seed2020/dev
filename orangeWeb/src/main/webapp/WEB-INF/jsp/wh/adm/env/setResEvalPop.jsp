...
function getCheckedTrs(noSelectMsg){
	var arr=[], id, obj;
	$("#evalListArea tbody:first input[type='checkbox']:checked").each(function(){
...
<%// 다음 Row 번호 %>
var gMaxRow = parseInt('${fn:length(whResEvalBVoList)}');
<%// 행추가%>
function addCatRow(){
	restoreUniform('evalListArea');
	var $tr = $("#evalListArea tbody:first #hiddenTr");
	...
	$tr.attr('id','');
	$tr.attr('style','');
	applyUniform('evalListArea');
	//setJsUniform($tr[0]);
}
<%// 행삭제%>
function delCatRow(){
	var $tr = $("#evalListArea tbody:first #hiddenTr").prev();
	delCatRowInArr([$tr[0]]);
}
<%// 선택삭제%>
...
function delCatRowInArr(rowArr){
	...
	for(var i=0;i<rowArr.length;i++){
		$mdId = $(rowArr[i]).find("input[name='evalNo']");
		if($mdId.val()!=''){
...
function changeLangSelectorCat(areaId, id, va){
	if(va==''){
		...
		var nm = $("#evalListArea #"+id).attr("name");
...
function saveCat() {
	...
	$("#evalListArea tbody:first").children().each(function(){
		trArr.push(this);
	});
	...
	<%// 정렬순서 세팅%>
	var ordr = 1;
	$("#evalListArea input[name='sortOrdr']").each(function(){
		if($(this).attr('id')!='sortOrdr_NO'){
			$(this).val(ordr++);
		}
	});
	<%// disable 된 select disable 해제 %>
	var arr = releaseDisable($('#evalListArea select'));
	
	var $form = $('#setForm');
	$form.attr('method','post');
	$form.attr('action','./transResEvalList.do?menuId=${menuId}');
...
<u:title titleId="wh.jsp.resEval.title" type="small" alt="결과평가" >
...
<u:listArea id="evalListArea" style="height:220px;overflow-y:auto;position:relative;" colgroup="15%,">	
	<tr id="headerTr">
		<th class="head_ct"><input type="checkbox" id="checkHeader" title="<u:msg titleId='cm.check.all' />" onclick="checkAllCheckbox('evalListArea', this.checked);" value=""/></th>
		<th class="head_ct"><u:mandatory /><a href="javascript:changeLangTypCds('evalListArea');" title="<u:msg titleId="pt.jsp.terms.chgLangAll" alt="일괄 언어 변경" />"><u:msg titleId="wh.cols.eval.nm" alt="평가명"/></a></th>
	</tr>
	<u:set var="nodataStyle" test="${fn:length(whResEvalBVoList)>1}" value="style=\"display:none;\"" elseValue=""/>
	<tr id="nodata" ${nodataStyle }>
	<td class="nodata" colspan="2"><u:msg titleId="cm.msg.noData" alt="해당하는 데이터가 없습니다." /></td>
	</tr>
	<c:if test="${fn:length(whResEvalBVoList)>0}">
		<c:forEach items="${whResEvalBVoList}" var="vo" varStatus="status"
			 ><u:set test="${status.last}" var="trDisp" value="display:none"
			/><u:set test="${status.last}" var="trId" value="hiddenTr" elseValue="tr${vo.evalNo}"
			...
			<tr onmouseover="this.className='trover'" onmouseout="this.className='trout'" id="${trId}" style="${trDisp}" >
				<td class="bodybg_ct"><input type="checkbox" id="listChk" value="${vo.evalNo }" /><u:input type="hidden" id="evalNo" value="${vo.evalNo }"/></td>
		...
		<u:input id="rescVa_${langTypCdVo.cd}${lstIndex}" name="rescVa_${langTypCdVo.cd}" titlePrefix="${titlePrefix}" titleId="wh.jsp.resEval.title" value="${rescVa}" style="${style}"
			maxByte="200" validator="changeLangSelectorCat('evalListArea', id, va)" mandatory="Y" />
		</c:forEach>
		</td>
		<td>
		<c:if test="${fn:length(_langTypCdListByCompId)>1}">
			<select id="langSelector${lstIndex}" onchange="changeLangTypCd('evalListArea','langTyp${lstIndex}', this.value)" <u:elemTitle titleId="cols.langTyp" />>
			<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="langStatus">
