...
<%// 다음 Row 번호 %>
var gMaxRow = parseInt('${fn:length(whMdBVoList)}');
<%// 행추가%>
function addRow(){
...
function delRowInArr(rowArr){
	...
	for(var i=0;i<rowArr.length;i++){
		$mdId = $(rowArr[i]).find("input[name='mdId']");
		if($mdId.val()!=''){
...
function saveMd(){
	...
	var arr = releaseDisable($('#listForm select'));
	
	<%// 서버 전송%>
	var $form = $("#listForm");
	$form.attr('method','post');
	$form.attr('action','./transMdList.do');
...
<%// 저장 후 리로드%>
function afterTrans(mdPid){
	parent.getIframeContent('mdTree').reloadFrame(mdPid);
	//parent.openListFrm(mdPid);
}
<%// 일괄 언어 선택 변경%>
...
<form id="listForm" style="padding:10px;">
<input type="hidden" name="menuId" value="${menuId}" /><c:if test="${not empty param.mdPid}">
<u:input type="hidden" id="mdPid" value="${param.mdPid}" /></c:if>
<u:listArea id="listArea" >
	...
	<tr id="headerTr">
		...
		<th width="35%" class="head_ct"><u:mandatory /><a href="javascript:changeLangTypCds('listForm');" title="<u:msg titleId="pt.jsp.terms.chgLangAll" alt="일괄 언어 변경" />"><u:msg titleId="wh.cols.md.nm" alt="모듈명" /></a></th>
		<th width="15%" class="head_ct"><u:msg titleId="wh.cols.md.grp" alt="모듈그룹" /></th>
	...
	<c:forEach items="${whMdBVoList}" var="whMdBVo" varStatus="status"
			...
			/><u:set test="${status.last}" var="trId" value="hiddenTr" elseValue="tr${whMdBVo.mdId}"
	...
	<tr onmouseover="this.className='trover'" onmouseout="this.className='trout'" id="${trId}" style="${trDisp}" >
		<td class="bodybg_ct"><input type="checkbox" value="${whMdBVo.mdId}"/></td>
		<td>
			...
			<u:input id="rescVa_${langTypCdVo.cd}${lstIndex}" name="rescVa_${langTypCdVo.cd}" titlePrefix="${titlePrefix}" titleId="cols.clsNm" value="${rescVa}" style="${style}"
				maxByte="200" validator="changeLangSelectorCls('listArea', id, va)" mandatory="Y" />
			</c:forEach>
			</td>
			<td>
			<c:if test="${fn:length(_langTypCdListByCompId)>1}">
				<select id="langSelector${lstIndex}" onchange="changeLangTypCd('listArea','langTyp${lstIndex}', this.value)" <u:elemTitle titleId="cols.langTyp" />>
				<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="langStatus">
				<u:option value="${langTypCdVo.cd}" title="${langTypCdVo.rescNm}" />
				</c:forEach>
				</select>
			</c:if>
			<u:input type="hidden" id="rescId${lstIndex}" name="rescId" value="${whMdBVo.rescId}" />
			</td>
			</tr>
			</table>
		</td><td class="listicon_ct"><select id="mdTypCd${lstIndex}" name="mdTypCd" <u:elemTitle titleId="wh.cols.md.grp" />>
		...
		</td><td class="listicon_ct"><select id="useYn${lstIndex}" name="useYn" <u:elemTitle titleId="cols.useYn" />>
			...
			<u:input type="hidden" id="mdId${lstIndex}" name="mdId" value="${whMdBVo.mdId }"/>
		</td>
	</tr>
	</c:forEach>
