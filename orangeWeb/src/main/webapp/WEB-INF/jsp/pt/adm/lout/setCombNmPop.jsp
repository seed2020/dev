...
<form id="setCombNmPop">
<input type="hidden" name="menuId" value="${menuId}" />
<u:input id="mode" type="hidden" value="${param.mode}" />
<u:listArea>

	<tr>
	<td width="35%" class="head_lt"><u:mandatory /><u:msg titleId="cols.mnuComb" alt="메뉴조합" /></td>
	<td>
		...
		<u:input id="rescVa_${langTypCdVo.cd}" titlePrefix="${titlePrefix}" titleId="cols.mnuComb" value="${rescVa}" style="${style}"
			maxByte="200" validator="changeLangSelector('setCombNmPop', id, va)" mandatory="Y" />
		</c:forEach>
		</td>
		<td>
		<c:if test="${fn:length(_langTypCdListByCompId)>1}">
			<select id="langSelector" onchange="changeLangTypCd('setCombNmPop','langTypArea',this.value)" <u:elemTitle titleId="cols.langTyp" />>
			<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="status">
...
<u:buttonArea>
	<u:button alt="확인" href="javascript:setCombNm('${param.mode}');" titleId="cm.btn.confirm" auth="A" />
