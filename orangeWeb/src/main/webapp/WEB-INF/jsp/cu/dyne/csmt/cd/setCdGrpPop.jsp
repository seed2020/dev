...
<%// 다음 Row 번호 %>
var gMaxRow = parseInt('${fn:length(weCdDVoList)}');
<%// 행추가%>
function addCdRow(){
...
<c:if test="${!empty weCdGrpBVo }">
<u:input type="hidden" id="cdGrpId" value="${weCdGrpBVo.cdGrpId }"/>
</c:if>
...
<% // 폼 필드 %>
	...
	<tr>
				...
				<tr>
					<td id="langTypArea">
						<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="status">
							<u:convert srcId="${weCdGrpBVo.rescId}_${langTypCdVo.cd}" var="rescVa" />
						...
						<u:input type="hidden" id="grpRescId" value="${weCdGrpBVo.rescId}" />
					</td>
		...
		<td><u:checkArea><u:radio name="cdUseYn" value="Y" titleId="cm.option.use" alt="사용" checkValue="${weCdGrpBVo.cdUseYn }"  inputClass="bodybg_lt" checked="${empty weCdGrpBVo.cdUseYn }"
		/><u:radio name="cdUseYn" value="N" titleId="cm.option.notUse" alt="사용안함" checkValue="${weCdGrpBVo.cdUseYn }" inputClass="bodybg_lt" /></u:checkArea></td></tr>
</u:listArea>
<u:blank />
...
<u:listArea id="cdListArea" style="height:220px;overflow-y:auto;" colgroup="15%,,25%">	
	...
	<u:set var="nodataStyle" test="${fn:length(weCdDVoList)>1}" value="style=\"display:none;\"" elseValue=""/>
	<tr id="nodata" ${nodataStyle }>
	<td class="nodata" colspan="3"><u:msg titleId="cm.msg.noData" alt="해당하는 데이터가 없습니다." /></td>
	</tr>
	<c:if test="${fn:length(weCdDVoList)>0}">
		<c:forEach items="${weCdDVoList}" var="vo" varStatus="status"
