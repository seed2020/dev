...
function saveCd(){
	...
	if(confirmMsg("cm.cfrm.save")){
		...
		$form.attr('action','/cm/transCd.do');
		$form.submit();		
	}
};
...
var gfPageNo;
//화면 표시된 갯수만큼 uniform 적용
function fnSetUniform(pageNo , pageRowCnt){
	var strtNo = (pageNo-1) * pageRowCnt;
	var endNo = pageNo * pageRowCnt;
	gfPageNo = pageNo +1;
	$("#cdListArea tbody:first").children("[id!='hiddenTr']").each(function(idx){
		if(idx > strtNo && idx <= endNo ){
			$(this).show();
			setJsUniform(this);	
		}
	});
...
function fnCdNext(){
	fnSetUniform(gfPageNo,'${pageRowCnt}');
};

$(document).ready(function() {
	<%// 행추가 영역 제외하고 uniform 적용%>
	fnSetUniform(1,'${pageRowCnt}');
	gfPageNo = 2;
	$('#schResultLoading').hide();
	if('${fn:length(ptCdBVoList)}' > 0 ){
		parent.btnDipYn('show');	
	}
});
...
<h4 id="schResultLoading" ><u:msg titleId="em.jsp.findZipCode.schStatus" alt="검색중입니다....."/></h4>
...
<u:listArea id="cdListArea" >
	...
	<tr id="headerTr">
		...
		<th width="12%" class="head_ct"><u:msg titleId="cols.compIds" alt="회사ID목록" /></th>
		<th width="8%" class="head_ct"><u:msg titleId="cols.useYn" alt="사용여부" /></th>
	</tr>
	
	<c:forEach items="${ptCdBVoList}" var="ptCdBVo" varStatus="status"
			 ><u:set test="${status.last || status.index+1 > pageRowCnt}" var="trDisp" value="display:none"
			...
			<td>
				...
				<u:convert srcId="${ptCdBVo.cd}_${langTypCdVo.cd}" var="rescVa" />
				...
				<u:input type="hidden" id="rescId${cdIndex}" name="rescId"  />
				</td>
				</tr>
				</table>
			</td>
			...
			<td><u:input id="cdDesc${cdIndex}" name="cdDesc" value="${ptCdBVo.cdDesc}" titleId="cols.cdDesc" maxByte="400" style="width:88%" /></td>
			<td><u:input id="compIds${cdIndex}" name="compIds" value="${ptCdBVo.compIds}" titleId="cols.compIds" maxByte="120" style="width:84%" /></td>
...
<input type="hidden" id="clsCd" name="clsCd" value="${clsCd}" />
<input type="hidden" id="delList" name="delList" />
<c:set var="pages" value="${recodeCount / pageRowCnt }"/>

<u:buttonArea>
	<u:button titleId="cm.btn.next" onclick="fnCdNext();" alt="다음" />
</u:buttonArea>
