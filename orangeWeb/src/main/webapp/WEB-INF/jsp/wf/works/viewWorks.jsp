...
<c:if test="${!empty mdCd && !empty isOnlyMd && isOnlyMd==true && (mdCd eq 'AP' || (isMobile==true && mdCd ne 'AP'))}">
<style type="text/css">
.titlearea {
    width: 100%;
    height: 16px;
...
<!-- 
<c:if test="${!empty mdCd && mdCd eq 'AP' && !empty isOnlyMd && isOnlyMd==true}">
//탭변경 무시
...
function setWorks(workNo){
	var url="./setWorks.do?${paramsForList}&workNo=${param.workNo}";
	parent.location.href=url;
}<% // [하단버튼:삭제] 삭제 %>
function delWorks() {
	if (confirmMsg("cm.cfrm.del")) {<% // 삭제하시겠습니까? %>
		callAjax('./transWorksDelAjx.do?menuId=${menuId}', {formNo:'${param.formNo}', workNos:['${param.workNo}']}, function(data) {
			if (data.message != null) {
				alert(data.message);
			}
			if (data.result == 'ok') {
				location.href = './listWorks.do?${paramsForList}';
			}
		});
	}
}
...
$(document).ready(function() {
	try{setUniformCSS();	}catch(e){}
});
//-->
</script>
<u:set var="jspInclPath" test="${!empty isOnlyMd && isOnlyMd==true && isMobile==true }" value="web" elseValue="works"/>
...
<c:if test="${isOpen==true && ( empty isOnlyMd || isOnlyMd==false)}"><c:set var="style" value=" style=\"min-width:1080px;padding:10px;\""/></c:if>
<c:if test="${!empty isOnlyMd && isOnlyMd==true && isMobile==true }"><c:set var="style" value=" style=\"min-width:1080px;\""/></c:if>
<div${style }>
<c:if test="${empty isOnlyMd || isOnlyMd==false}"><c:if test="${empty isPop }"><u:title title="${wfFormBVo.formNm }" alt="${wfFormBVo.formNm }" menuNameFirst="true" /></c:if></c:if>
