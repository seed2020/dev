...
<c:if test="${fn:length(wfCdGrpBVoList) == 0}">
	<tr>
	<td class="nodata" colspan="5"><u:msg titleId="cm.msg.noData" alt="해당하는 데이터가 없습니다." /></td>
	</tr>
</c:if>
<c:if test="${fn:length(wfCdGrpBVoList) > 0}">
	<c:forEach items="${wfCdGrpBVoList}" var="wfCdGrpBVo" varStatus="status">
	<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
	<td class="bodybg_ct"><input type="checkbox" value="${wfCdGrpBVo.cdGrpId}"/></td>
	<td class="body_lt"><a href="javascript:setCdGrpPop('${wfCdGrpBVo.cdGrpId}');">${wfCdGrpBVo.cdGrpRescNm}</a></td>
	<td class="body_lt">
	<c:if test="${fn:length(wfCdGrpBVo.wfCdDVoList) > 0}">
		<c:forEach items="${wfCdGrpBVo.wfCdDVoList}" var="wfCdDVo" varStatus="status">
		<c:if test="${status.count > 1 }">,</c:if>
		${wfCdDVo.cdRescNm}</c:forEach>
	</c:if></td>
	<td class="body_ct"><u:yn value="${wfCdGrpBVo.cdUseYn }" yesId="cm.option.use" noId="cm.option.notUse" alt="사용여부"/></td>
	<td class="body_ct"><u:out value="${wfCdGrpBVo.regDt}" type="date" /></td>
	</tr>
	</c:forEach>
</c:if>
</u:listArea>
