...
<c:if test="${fn:length(baScreHstLVoList) == 0}">
	<tr>
	<td class="nodata" colspan="5"><u:msg titleId="cm.msg.noData" alt="해당하는 데이터가 없습니다." /></td>
	</tr>
</c:if>
<c:if test="${fn:length(baScreHstLVoList) > 0}">
	<c:forEach items="${baScreHstLVoList}" var="baScreHstLVo" varStatus="status">
	<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
	<td class="body_ct"><c:forEach begin="1" end="5" step="1" varStatus="status"><u:set test="${status.count <= baScreHstLVo.scre}" var="star" value="★" elseValue="☆" />${star}</c:forEach></td>
	<td class="body_ct"><a href="javascript:viewUserPop('${baScreHstLVo.userUid}');">${baScreHstLVo.orUserBVo.rescNm}(${baScreHstLVo.orOdurBVo.lginId})</a></td>
	<td class="body_ct">${baScreHstLVo.orUserBVo.deptRescNm}</td>
	<td class="body_ct">${baScreHstLVo.orUserBVo.gradeNm}</td>
	<td class="body_ct"><u:out value="${baScreHstLVo.regDt}" type="longdate" /></td>
	</tr>
	</c:forEach>
</c:if>
</u:listArea>
