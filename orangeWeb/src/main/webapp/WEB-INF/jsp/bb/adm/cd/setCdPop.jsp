...
<!--
<% // 다음 Row 번호 %>
var gMaxRow = parseInt('${fn:length(baCdGrpBVo.baCdDVoList)}');
<% // [목록버튼:추가] TR 추가 %>
function addRow() {
...
function delRow(btn) {
	...
	if($cdId.val() != '' && $('#setCdForm').find('input[id="cdGrpId"]').val() != ''){
		callAjax('./chkUseCdAjx.do?menuId=${menuId}', {cdGrpId:'${baCdGrpBVo.cdGrpId}',cdId:$cdId.val()}, function(data) {
...
function delCd() {
	if(confirmMsg("cm.cfrm.del")) {	<% // cm.cfrm.del=삭제하시겠습니까? %>
		callAjax('./transCdDelAjx.do?menuId=${menuId}', {cdGrpId:'${baCdGrpBVo.cdGrpId}'}, function(data) {
			if (data.message != null) {
				alert(data.message);
			}
			if (data.result == 'ok') {
...
function useSelect(obj,cdId){
	if(cdId != '' && $('#setCdForm').find('input[id="cdGrpId"]').val() != '' && $(obj).val() =='N'){
		// bb.msg.not.use.cd=사용중인 코드는 사용안함 처리 할 수 없습니다.
		callAjax('./chkUseCdAjx.do?menuId=${menuId}', {cdGrpId:'${baCdGrpBVo.cdGrpId}',cdId:cdId,msgCode:'bb.msg.not.use.cd'}, function(data) {
			if (data.message != null) {
				alert(data.message);
				$(obj).val('Y');
			}
...
$(document).ready(function() {
	<c:if test="${empty baCdGrpBVo.cdGrpId}">
	addRow();
	</c:if>
});
//-->
...
<u:input type="hidden" id="cdGrpId" value="${baCdGrpBVo.cdGrpId}" />
...
<u:listArea colgroup="27%,73%">
	...
	<td>
		<select id="compId" name="compId" <u:elemTitle titleId="cols.comp" /> <c:if test="${!empty baCdGrpBVo.cdGrpId }">disabled="disabled"</c:if>>
	...
	<td><table cellspacing="0" cellpadding="0" border="0">
		...
		<td id="langTypArea">
		<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="status">
			<u:convert srcId="${baCdGrpBVo.rescId}_${langTypCdVo.cd}" var="grpRescVa" />
		...
		<u:input type="hidden" id="grpRescId" value="${baCdGrpBVo.rescId}" />
		</td>
		</tr>
		</table></td>
	</tr>
	<tr>
		...
		<td>
			<u:checkArea>
				<u:radio name="grpUseYn" value="Y" titleId="cm.option.use" checked="${empty baCdGrpBVo.grpUseYn || baCdGrpBVo.grpUseYn eq 'Y' ? true : false}" />
				<u:radio name="grpUseYn" value="N" titleId="cm.option.notUse" checked="${baCdGrpBVo.grpUseYn eq 'N' ? true : false }" />
			</u:checkArea>
		</td>
	</tr>
</u:listArea>
...
<u:listArea id="cdList" colgroup="33,,100,60">
	<c:forEach items="${baCdGrpBVo.baCdDVoList}" var="cdVo" varStatus="status">
