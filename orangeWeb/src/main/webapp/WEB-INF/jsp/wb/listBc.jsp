...
<u:set test="${param.typ eq 'L' || wbBcUserScrnSetupRVo.lstTypCd eq 'LIST' || listPage eq 'listAllBc'}" var="getCheckedTrs" value="getCheckedTrs" elseValue="getCheckedDivs"/>
...
function setEmailSend(){
	...
	emailSendPop({bcIds:bcIds},'${menuId }');
};
...
function openSingUser(){
	...
	//dialog.open('copyBcPop','명함복사','/wb/copyBcPop.do?menuId=${menuId}');
	var data = {};<%// 팝업 열때 선택될 데이타 %>
	var userArr = [];
	<%// option : data, multi, withSub, titleId %>
	searchUserPop({data:data}, function(userVo){
...
function viewBc(bcId) {
	location.href="./${viewPage}.do?${paramsForList }&typ=${param.typ}${agntParam}&bcId="+bcId;
};
			...
			<table border="0" cellpadding="0" cellspacing="0">
				<tbody>
					<tr>
						<td class="frontbtn">
							<c:choose>
								<c:when test="${param.typ eq 'C' || ( empty wbBcUserScrnSetupRVo && empty param.typ )|| ( empty param.typ && wbBcUserScrnSetupRVo.lstTypCd eq 'BC' )}"><u:buttonS href="./${listPage }.do?${paramsForList }&typ=L${agntParam }" titleId="wb.btn.txtView" alt="리스트로보기" /></c:when>
								<c:otherwise><u:buttonS href="./${listPage }.do?${paramsForList }&typ=C${schParam }" titleId="wb.btn.bcView" alt="명함으로보기" /></c:otherwise>
							</c:choose>
						</td>
					</tr>
				</tbody>
...
<c:choose>
	<c:when test="${!empty wbBcBMapList}">
		<div id="bcListArea" style="position:relative; width:100%; height:313px; background:#ffffff; border:1px solid #bfc8d2; overflow-y:auto; overflow-x:auto;">
			<c:forEach var="wbBcBMap" items="${wbBcBMapList}" varStatus="status">
				<c:set var="wbBcBMap" value="${wbBcBMap}" scope="request" />
				<div class="cardbox" style="width:31.7%;">
					<div class="ptltit">
						<dl>
...
<c:if test="${listPage ne 'listAllBc' && mailEnable == 'Y'}">
	<u:button titleId="cm.btn.emailSend" alt="이메일발송" href="javascript:setEmailSend();" />
</c:if>
<c:if test="${listPage eq 'listBc' || ( listPage eq 'listAgntBc' && wbBcAgntAdmBVo.authCd eq 'RW' ) || listPage eq 'listAllBc'}">
	<u:button titleId="cm.btn.copy" alt="복사" href="javascript:openSingUser();" auth="${listPage eq 'listAllBc' ? 'A' : 'W'}" />
	<c:if test="${listPage ne 'listAllBc' }"><u:button titleId="cm.btn.reg" alt="등록" href="./${setPage }.do?${paramsForList }&typ=${param.typ}${agntParam }" auth="W" /></c:if>
