...
function saveBbPltSetup(){
	var arr = [];
	$("#WbBcPltSetupArea input[name='BcPltSetupCheck']:checked").each(function(){
		arr.push($(this).val());
	});
	if(arr.length>5){<%
		...
		var result = false;
		callAjax("${_cxPth}/wb/plt/transBcBxPltSetupAjx.do?menuId=${menuId}", {pltId:'${param.pltId}', bxIds:arr.join(',')}, function(data){
...
<u:listArea id="WbBcPltSetupArea" colgroup="7%,93%" style = "height:257px; overflow-x:hidden; overflow-y:auto;">
	<tr id="titleTr"><td class="head_bg"><input type="checkbox" id="checkHeader" title="<u:msg titleId='cm.check.all'
		...
		<td class="head_ct"><u:msg titleId="wb.cols.bc" alt="명함" /></td>
	</tr>
<c:forEach items="${wbBcPltSetupDVoList}" var="wbBcPltSetupDVo" varStatus="status">
	<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
		<td class="bodybg_ct"><input type="checkbox" name="BcPltSetupCheck" id="${wbBcPltSetupDVo.bxId}BbPltSetup"
			value="${wbBcPltSetupDVo.bxId}" ${wbBcPltSetupDVo.useYn=='Y' ? 'checked="checked"' : ''} /></td>
		<td class="body_lt"><label for="${wbBcPltSetupDVo.bxId}BbPltSetup"><u:titleText title="${wbBcPltSetupDVo.bxNm }" noLi="true"/></label></td>
	</tr>
</c:forEach>
</u:listArea>
