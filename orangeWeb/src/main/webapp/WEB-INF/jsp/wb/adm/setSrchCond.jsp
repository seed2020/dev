...
<%// 초기화%>
function doReset(){
	if(confirmMsg("wb.cfrm.setScrn.reset")) {<%//wb.cfrm.setScrn.reset=설정을 초기화 하시겠습니까? %>
		var id;
		var arr = [];
		$('#listArea tbody:first tr').each(function(){
			id = $(this).attr('id');
			if(id!='headerTr' && id!='hiddenTr') arr.push(this);
		});
...
function save(){
	var cnt = 0;
	$("#listArea select[name=dispYn]").each(
		function(){
			if($(this).val() == "Y") cnt++;
		}
	);
	if(cnt > 7 ){
		alert("명함표시여부는 최대 6개까지만 설정이 가능합니다.");
		return;
	}	
	...
	<%// 서버 전송%>
	if ( ( etcValidator && confirmMsg("wb.cfrm.setBcScrn.wdth") ) || ( !etcValidator && confirmMsg("cm.cfrm.save") ) ) {
		var $form = $('#setSrchCond');
		$form.attr('method','post');
		$form.attr('target','dataframe');
		$form[0].submit();
...
<u:title titleId="wb.jsp.setSrchCond.title" alt="명함 검색조건 설정" menuNameFirst="true"/>

<form id="setSrchCond" action="./transSrchCond.do?menuId=${menuId}">
				...
				<c:if test="${!empty wbBcUserLstSetupRVoList}">
					<c:forEach var="list" items="${wbBcUserLstSetupRVoList}" varStatus="status">
						...
						<tr onmouseover="this.className='trover'" onmouseout="this.className='trout'" id="${trId}" style="${trDisp}">
							...
							<td class="body_lt" id="msgNm"><u:term termId="${list.msgId}" /></td>
							<td class="bodybg_ct">
							...
							<td class="bodybg_ct">
								<select id="dispYn" name="dispYn"<u:elemTitle titleId="wb.cols.dispYn" alt="명함표시여부(최대6개)" /> >
