...
%>
<u:set var="agntParam" test="${!empty param.schBcRegrUid }" value="&schBcRegrUid=${param.schBcRegrUid }" elseValue=""/>
...
function delFld(){
	...
	if(sel==null){
		...
		return;
	} else if($(sel).attr('id')=='ROOTLI' || $(sel).attr('id')=='NONELI'){
	...
	if(exts==null){
		...
		callAjax('./transBcFldDelAjx.do?menuId=${menuId}${agntParam}', {mode:'delete', abvFldId:exts['pid'], bcFldId:exts['id']}, function(data){
			...
			if(data.result=='ok'){
				...
				if($next.length>0){
					...
					//reload('./${listPage }FldFrm.do?menuId=${menuId}${agntParam }&bcFldId='+id);
					...
					//reload('./${listPage }FldFrm.do?menuId=${menuId}${agntParam }&bcFldId='+exts['pid']);
				}
			}
...
function getTreeData(forOption){
	var sel = TREE.getTree('cdTree').selected;
	if($(sel).attr('id')=='ROOTLI' || $(sel).attr('id')=='NONELI'){
		if(forOption=='mod'){
