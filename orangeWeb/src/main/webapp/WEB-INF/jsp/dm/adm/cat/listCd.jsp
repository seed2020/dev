...
<c:if test="${fn:length(dmCdGrpBVoList) == 0}">
	<tr>
	<td class="nodata" colspan="5"><u:msg titleId="cm.msg.noData" alt="해당하는 데이터가 없습니다." /></td>
	</tr>
</c:if>
<c:if test="${fn:length(dmCdGrpBVoList) > 0}">
	<c:forEach items="${dmCdGrpBVoList}" var="dmCdGrpBVo" varStatus="status">
	<tr onmouseover='this.className="trover"' onmouseout='this.className="trout"'>
	<td class="body_lt"><a href="javascript:modCd('${dmCdGrpBVo.cdGrpId}');">${dmCdGrpBVo.rescNm}</a></td>
	<td class="body_lt">
	<c:if test="${fn:length(dmCdGrpBVo.dmCdDVoList) > 0}">
		<c:forEach items="${dmCdGrpBVo.dmCdDVoList}" var="baCatDVo" varStatus="status">
		<c:if test="${status.count > 1 }">,</c:if>		
		${baCatDVo.rescNm}
		</c:forEach>
	</c:if></td>
	<td class="body_ct"><u:set var="useMsgCd" test="${dmCdGrpBVo.grpUseYn eq 'Y'}" value="cm.option.use" elseValue="cm.option.notUse"/><u:msg titleId="${useMsgCd }" alt="사용여부" /></td>
	<td class="body_ct"><u:out value="${dmCdGrpBVo.regDt}" type="date" /></td>
	</tr>
	</c:forEach>
</c:if>
</u:listArea>
