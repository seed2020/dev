...
function saveFld(){
	...
	$form.attr('action','./transDocFldList.do');
...
<u:listArea id="fldListArea" >
	...
	<tr id="headerTr">
		...
		<th class="head_ct"><u:msg titleId="dm.cols.cat" alt="문서유형" /></th>
	...
	<tr onmouseover="this.className='trover'" onmouseout="this.className='trout'" id="${trId}" style="${trDisp}" >
		...
		<td><select id="catId${fldIndex}" name="catId" <u:elemTitle titleId="dm.cols.cat" /> style="width:300px;">
			<c:forEach items="${dmCatBVoList}" var="dmCatBVo" varStatus="compStatus">
			<u:option value="${dmCatBVo.catId}" title="${dmCatBVo.catNm}" checkValue="${dmFldBVo.catId}"
				/></c:forEach></select></td>
