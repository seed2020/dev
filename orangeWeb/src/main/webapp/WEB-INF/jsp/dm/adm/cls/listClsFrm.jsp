...
function getCheckedTrs(noSelectMsg){
	var arr=[], id, obj;
	$("#clsListArea tbody:first input[type='checkbox']:checked").each(function(){
...
<%// 다음 Row 번호 %>
var gMaxRow = parseInt('${fn:length(dmClsBVoList)}');
<%// 행추가%>
function addRow(){
	var $tr = $("#clsListArea tbody:first #hiddenTr");
...
<%// 행삭제%>
function delRow(){
	var $tr = $("#clsListArea tbody:first #hiddenTr").prev();
	delRowInArr([$tr[0]]);
}
<%// 선택삭제%>
function delSelRow(){
...
function delRowInArr(rowArr){
	var delVa = $("#delList").val(), delArr = [], $clsId;
	if(delVa!='') delArr.push(delVa);
	for(var i=0;i<rowArr.length;i++){
		$clsId = $(rowArr[i]).find("input[name='clsId']");
		if($clsId.val()!=''){
			delArr.push($clsId.val());
		}
		$(rowArr[i]).remove();
	}
...
function changeLangSelectorCls(areaId, id, va){
	if(va==''){
		var langSelector = $('#'+areaId+' #langSelector'+id.substring(id.lastIndexOf('_')));
		var nm = $("#clsListArea #"+id).attr("name");
...
function saveCls(){
	...
	$("#clsListArea tbody:first").children().each(function(){
		trArr.push(this);
	});
	...
	<%// 정렬순서 세팅%>
	var ordr = 1;
	$("#clsListArea input[name='sortOrdr']").each(function(){
		if($(this).attr('id')!='sortOrdr_NO'){
			$(this).val(ordr++);
		}
	});
	<%// disable 된 select disable 해제 %>
	var arr = releaseDisable($('#clsListForm select'));
	
	<%// 서버 전송%>
	var $form = $("#clsListForm");
	$form.attr('method','post');
	$form.attr('action','./transClsList.do');
...
<%// 저장 후 리로드%>
function afterTrans(clsPid){
	parent.getIframeContent('clsTree').reoladFrame(clsPid);
	parent.openListFrm(clsPid);
}
<%// 일괄 언어 선택 변경%>
...
function getCheckedArray(){
	var arr = [], va;
	$("#clsListArea input[type=checkbox]:checked").each(function(){
		va = $(this).val();
...
function pasteArray(cutInfo){
	callAjax('./transClsPasteAjx.do?menuId=${menuId}', cutInfo, function(data){
		if(data.message!=null){
			alert(data.message);
		}
		if(data.clsId != null){
			parent.getIframeContent('clsTree').reoladFrame(data.clsId);
			reload();
		}
	});
}
...
<form id="clsListForm" style="padding:10px;">
<input type="hidden" name="menuId" value="${menuId}" /><c:if test="${not empty param.clsPid}">
<u:input type="hidden" id="clsPid" value="${param.clsPid}" /></c:if>
<u:listArea id="clsListArea" >

	<tr id="headerTr">
		<th width="3%" class="head_bg"><input type="checkbox" id="checkHeader" title="<u:msg titleId='cm.check.all' />" onclick="checkAllCheckbox('clsListArea', this.checked);" value=""/></th>
		<th width="35%" class="head_ct"><u:mandatory /><a href="javascript:changeLangTypCds('clsListForm');" title="<u:msg titleId="pt.jsp.terms.chgLangAll" alt="일괄 언어 변경" />"><u:msg titleId="cols.clsNm" alt="폴더명" /></a></th>
	...
	<c:forEach items="${dmClsBVoList}" var="dmClsBVo" varStatus="status"
			...
			/><u:set test="${status.last}" var="trId" value="hiddenTr" elseValue="tr${dmClsBVo.clsId}"
	...
	<tr onmouseover="this.className='trover'" onmouseout="this.className='trout'" id="${trId}" style="${trDisp}" >
		<td class="bodybg_ct"><input type="checkbox" value="${dmClsBVo.clsId}"/></td>
		<td>
			<table cellspacing="0" cellpadding="0" border="0">
			<tr>
			<td id="langTyp${clsIndex}">
			<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="langStatus">
			<u:convert srcId="${dmClsBVo.rescId}_${langTypCdVo.cd}" var="rescVa" />
			...
			<u:input id="rescVa_${langTypCdVo.cd}${clsIndex}" name="rescVa_${langTypCdVo.cd}" titlePrefix="${titlePrefix}" titleId="cols.clsNm" value="${rescVa}" style="${style}"
				maxByte="200" validator="changeLangSelectorCls('clsListArea', id, va)" mandatory="Y" />
			</c:forEach>
			</td>
			<td>
			<c:if test="${fn:length(_langTypCdListByCompId)>1}">
				<select id="langSelector${clsIndex}" onchange="changeLangTypCd('clsListArea','langTyp${clsIndex}', this.value)" <u:elemTitle titleId="cols.langTyp" />>
				<c:forEach items="${_langTypCdListByCompId}" var="langTypCdVo" varStatus="langStatus">
				<u:option value="${langTypCdVo.cd}" title="${langTypCdVo.rescNm}" />
				</c:forEach>
				</select>
			</c:if>
			<u:input type="hidden" id="rescId${clsIndex}" name="rescId" value="${dmClsBVo.rescId}" />
			</td>
			</tr>
			</table>
		</td>
		<td class="listicon_ct"><select id="useYn${clsIndex}" name="useYn" <u:elemTitle titleId="cols.useYn" />>
			<u:option value="Y" titleId="cm.option.use" checkValue="${dmClsBVo.useYn}" />
			<u:option value="N" titleId="cm.option.notUse" checkValue="${dmClsBVo.useYn}" />
			</select>
			<u:input type="hidden" id="sortOrdr${clsIndex}" name="sortOrdr" />
			<u:input type="hidden" id="valid${clsIndex}" name="valid" />
			<u:input type="hidden" id="clsId${clsIndex}" name="clsId" value="${dmClsBVo.clsId }"/>
		</td>
	</tr>
	</c:forEach>
