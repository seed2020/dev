...
function setSaveVacation(isHidden){
	if(isHidden===undefined || !isHidden) return;
	// 휴가 키 목록
	var buffer=[];
...
function loadUserVacation(isSave){
	callAjax('./getAnbAjx.do?menuId=${menuId}&bxId=${param.bxId}', {odurUid:$('#erpOdurUid').val()}, function(data) {
		if (data.message != null) {
			...
			// 휴가 키 목록
			var vacationKeys=['anb', 'anbMinus', 'anbOngo', 'newYearMonth', 'newYearDay'];
			userVacation={};
...
<td class="body_lt"><table border="0" cellpadding="0" cellspacing="0">
			<tr>
			<td><select id="erpOptions" name="erpOptions" onchange="chnErpOptions(this);">
			<c:forEach items="${erpOptions}" var="erpOption" varStatus="status">
...
<td class="body_lt" ><u:input id="erpUserNm" value="${erpUserNm }" title="신청자명" style="width:100px;" maxByte="100" mandatory="Y" readonly="Y"/></td>
</tr>
...
<tr><td class="head_ct"><u:mandatory />휴가 신청자</td>
<td class="body_lt" ><u:out value="${formBodyXML.getAttr('body/leave.erpUserNm') }"/></td>
</tr>
<tr><td class="head_ct"><u:mandatory />휴가 사유</td>
