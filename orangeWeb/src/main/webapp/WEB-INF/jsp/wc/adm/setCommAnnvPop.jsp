...
function findOrgc() {
	dialog.open('findOrgcPop','조직도','/bb/findOrgcPop.do?menuId=${menuId}');
}
...
function formSubit(){
	...
	if(validator.validate('setAnnvForm')){
		...
		$form.attr('action','./transSetCommAnnvPopSave.do?menuId=${menuId}');
...
$(document).ready(function() {
	...
	$("#startYear").val(currentYear).attr("selected", "selected");
...
<div class="front">
	<div class="front_left">
		<tr>
		<td class="red_stxt">* <u:msg titleId="wc.jsp.setCommAnnvModePop.tx01" alt="같은 날짜의 공통기념일은 하나만 표시됩니다.(우선순위 : 공휴일, 평일 [음력 우선])" /></td>
		</tr>
	</div>
</div>
...
<u:listArea noBottomBlank="true">
	<tr>
		...
		<u:input type="hidden" id="rescId" value="${wcAnnvDVo.rescId}" />
		</td>
		</tr>
		</table></td>
	</tr>
	<tr id="grpRow">
		<td width="18%" class="head_lt"><u:msg titleId="wc.btn.grpChoi" alt="그룹선택" /></td>
		<td colspan="3">
	...
	<td width="33%">
		...
		<table border="0" cellpadding="0" cellspacing="0"><tbody>
			...
			<td><u:calendar id="setAnnvChoiDt"  option="{end:'setAnnvCmltDt'}"  handler="function(date,option){compareDate(date);return true;}"  titleId="cols.choiDt" alt="선택일시" mandatory="Y" />
			<td class="body_lt"><u:msg titleId="wc.jsp.viewWorkPop.tx01" alt="부터" /></td>
			<td><select name = "selectStartYmd" onChange="selectedSevenDay()" >
	...
	<td width="33%"><table border="0" cellpadding="0" cellspacing="0"><tbody>
		...
		<td><u:calendar id="setAnnvCmltDt" readonly="Y" titleId="cols.cmltDt" alt="완료일시" />
		</tr>
		</tbody></table></td>
	</tr>
	...
	<td><u:checkArea>
		...
		<u:radio name="holiYn" value="Y" alt="휴일" titleId="wc.option.holi" checked="true"/>
		</u:checkArea></td>
	...
	<td colspan="3">
		<u:checkArea>
			<u:radio name="solaYn" value="Y" alt="양력" titleId="wc.option.sola" checked="true"/>
	...
	<td colspan="3"><select id="natCd" name="natCd" <u:elemTitle titleId="wc.btn.set.nat" alt="국가설정" />>
		<c:forEach items="${wcNatBVoList}" var="wcNatBVo" varStatus="status">
			<u:option value="${wcNatBVo.cd}" title="${wcNatBVo.rescNm}" selected="${status.first }"/>
		</c:forEach>
	</select></td>
	</tr>
