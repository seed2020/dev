...
<div style="width:400px;">
	...
	<c:if test="${!empty param.callback }"><u:input type="hidden" id="callback" value="${param.callback}" /></c:if>
	<% // 폼 필드 %>
	...
	<tr>
		...
		<td>
			...
			<u:option value="${list.cd }" title="${list.rescNm }" alt="${list.rescNm }" checkValue="${wcWorkSchdlBVo.schdlTypCd}"/>
			</c:forEach>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td><u:input type="hidden" id="userUid" value="${wcWorkSchdlBVo.userUid}"
		/><u:input id="userNm" titleId="wc.cols.schdlKndCd" readonly="Y"  value="${wcWorkSchdlBVo.userNm }" mandatory="Y"
	...
	<tr>
		...
		<td><u:input id="subj" value="${wcWorkSchdlBVo.subj }" titleId="cols.subj" style="width: 95%;" mandatory="Y" maxByte="120"/></td>
	</tr>
	<tr>
		<td class="head_lt"><u:mandatory /><u:msg titleId="wc.cols.date" alt="일자"/></td>
		<td><fmt:parseDate var="dateStartDt" value="${wcWorkSchdlBVo.strtDt}" pattern="yyyy-MM-dd HH:mm:ss"/>
	...
	<tr>
		<td colspan="2"><u:textarea id="cont" value="${wcWorkSchdlBVo.cont}" titleId="cols.cont" maxByte="1000" style="width:95.5%;resize:none;" rows="5" /></td>
	</tr>
	</u:listArea>
	...
	<c:if test="${!empty wcWorkSchdlBVo.schdlId }"><u:button titleId="cm.btn.del" onclick="javascript:delSchdl();" alt="삭제" auth="A"/></c:if>
