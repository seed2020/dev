...
$(function(){
	var grpInfo = schGrpInfo();
	$("#grpNm").val(grpInfo.schGrpNm);
	var $view = $("#selectedGrpMbsh");
	var schGrpMbshUid = [];
	var grpMbshUid = [];
	var grpMbshSchdlGrpId = [];
	var grpMbshNm = [];
	var i = 0;
	grpInfo.schGrpMbshUid.each(function(){
		schGrpMbshUid.push("<option>"+grpInfo.schGrpMbshUid[i]+"</option>");
		grpMbshUid.push("<input id='userUid' name ='userUid' type = 'hidden' value='"+grpInfo.grpMbshUid[i]+"'/>");
		grpMbshNm.push("<input id='rescNm' name ='rescNm' type = 'hidden' value='"+grpInfo.grpMbshNm[i]+"'/>");
		i++;
	});
	grpMbshSchdlGrpId.push("<input id='grpId' name ='grpId' type = 'hidden' value='"+grpInfo.grpMbshSchdlGrpId+"'/>");
	
	//$("#userInfo").html(grpMbshUid.join(''));
	$("#grpMbshSchdlGrpIds").html(grpMbshSchdlGrpId.join(''));
	//$("#grpMbshNms").html(grpMbshNm.join(''));
	//$view.html(schGrpMbshUid.join(''));
...
function formSubmit(){
	...
	$form.attr('action', './transSetGrpModPopUpdate.do?menuId=${menuId}');
	$form.submit();
}
...
<div id = "userUids" name = "userUids">
</div>
<div id = "grpMbshSchdlGrpIds" name = "grpMbshSchdlGrpIds">
</div>
<div id = "grpMbshNms" name = "grpMbshNms">
</div>
...
<u:listArea>
	...
	<td width="73%"><table border="0" cellpadding="0" cellspacing="0"><tbody>
		<tr>
		<td><u:input id="grpNm"  name="grpNm" value="${wcSchdlGroupB.grpNm }"  titleId="cols.grpNm" style="width: 194px;" /></td>
		</tbody></table></td>
	</tr>
	...
	<td><select id = "defaultAuth" name = "defaultAuth">
