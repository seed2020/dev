...
function formSubmit(){
	...
	if(validator.validate('survOendQuesForm')){
		...
		$form.attr('action', './transSetSurvOendQuesSave.do?menuId=${menuId}&fnc=${fnc}&survId=${survId}&quesId=${quesId}');
		$form.submit();
	}
}
...
<u:set test="${fnc == 'mod'}" var="subj" value="${wvSurvBVo.quesCont}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="selected" value=" selected" elseValue="" />
<u:set test="${fnc == 'mod'}" var="mandaReplyYn" value="${wvSurvBVo.mandaReplyYn}" elseValue="" />
...
<u:listArea id="oendQuesArea">
	<tr>
		...
		<td colspan="3">
			<u:input id="subj" value="${subj}" titleId="cols.subj" style="width: 98%;" mandatory="Y" onkeydown="if (event.keyCode == 13) return false;" maxLength="980"/>
		</td>
	</tr>
	...
	<tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td>				
					<input type="hidden" id="ques_img_file_id" name="ques_img_file_id" value="${wvSurvBVo.imgSurvFileId}" />
			...
			<tr>
				...
				<c:if test="${fnc == 'mod'}">
					<tr>
					<u:set test="${empty wvSurvBVo.imgSurvFileId}" var="subjDisp" value="none" elseValue="block" />
						<td class="td_imglt" style="display: ${subjDisp};" id="ques_img_mod_row"><img src="${_ctx}${wvSurvBVo.imgSavePath}" width="58" height="70"/></td>
					</tr>
				</c:if>
		</table>
		</td>
