...
function mulcSubmitForm(){
	...
	if(validator.validate('setMulcQuesPopForm')){
		...
		if(emptyChk != 1){   
			$form.attr('action','./setMulcQuesPopSave.do?menuId=${menuId}&fnc=${fnc}&survId=${survId}&quesId=${quesId}');
		   	$form.submit();
		  	dialog.close(this);
		}else{
...
function examAdd(){
	...
	var $data = "<tr class='item'><td class='bodybg_ct'><div class='checker' id='uniform-checkFlag'>"
				...
				+"<td><input id='exam' name='exam' title='<u:msg titleId="wv.cols.set.optInput" alt="보기 입력"/>' class='uniform-input text' style='width: 97%;' type='text' value='' maxlength='250'><script type='text/javascript'>"
...
<u:set test="${fnc == 'mod'}" var="subj" value="${wvsQuesVo.quesCont}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="imagePath" value="${wvsQuesVo.imgSavePath}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="examChoiItemCd" value="${wvsQuesVo.examChoiItemCd}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="inputYn" value="${wvQExamList[0].inputYn}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="mulChoiYn" value="${wvsQuesVo.mulChoiYn}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="examImgUseYn" value="${wvQExamList[0].examImgUseYn}" elseValue="" />
<u:set test="${fnc == 'mod'}" var="mandaReplyYn" value="${wvsQuesVo.mandaReplyYn}" elseValue="" />
...
<u:listArea id="mulcQuesArea">
	...
	<td colspan="3"><u:input id="subj" value="${subj}" titleId="cols.subj" style="width: 98%;"   mandatory="Y" maxByte="500" /></td>
	</tr>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td>				
				<input type="hidden" id="ques_img_file_id" name="ques_img_file_id" value="${wvsQuesVo.imgSurvFileId}">
		...
		<c:if test="${fnc == 'mod'}">
			<tr>
				<u:set test="${empty wvsQuesVo.imgSurvFileId}" var="subjDisp" value="none" elseValue="block" />
...
<u:listArea id="examArea">
	...
	<c:if test="${fnc == 'mod'}">
	<c:forEach  var="wvsQue" items="${wvQExamList}"  varStatus="status">
		...
		<tr class='item'>
			...
			<td>
				<input id='exam' name='exam' title='<u:msg titleId="wv.cols.set.optInput" alt="보기 입력"/>' class='uniform-input text' style='width: 97%;' type='text' value='${wvsQue.examDispNm}' maxlength='250'/>
				<script type='text/javascript'>
					$(document).ready(function() 
							{
			...
			<td style='display: table-cell;'>
				...
				<tbody>
					<tr class='item2'>
						<td class='td_btn'>
							<input type='hidden' name='exam_img_file_id' id='exam_img_file_id' value="${wvsQue.imgSurvFileId}">
								<a title='이미지추가' class='sbutton button_small' href='javascript:setImagePop( -1 , "${status.count}");'><span>이미지추가</span>
								</a>
						</td>
					</tr>
					<tr>
						<u:set test="${empty wvsQue.imgSurvFileId}" var="disp" value="none" elseValue="block" />
						<td class='td_imglt' id='exam_img_row' style='display: ${disp};'><img id='exam_img' src='${wvsQue.imgSavePath}' width='58' height='70'/>
