...
function returnSurv(){
	if($(':radio[name="apvdYn"]:checked').val() == "N"){
		$("#returnSurvDiv").css("display", "block");
	}else{
		$("#returnSurvDiv").css("display", "none");
	}
...
<u:title titleId="wv.jsp.viewSurvApvd.title" alt="설문승인" menuNameFirst="true"/>
...
<u:listArea>
	<tr>
		<td width="18%" class="head_lt"><u:msg titleId="cols.apvdYn" alt="승인여부" /></td>
		<td><u:checkArea>
			<u:radio name="apvdYn" value="Y" titleId="cm.option.apvd" inputClass="bodybg_lt"  checked="true" onclick="returnSurv()"/>
			<u:radio name="apvdYn" value="N" titleId="cm.option.rjt" inputClass="bodybg_lt" onclick="returnSurv()" />
			</u:checkArea>
...
<u:listArea id="returnSurvDiv" style="display:none">
	<tr>
		<td width="18%" class="head_lt"><u:msg titleId="cols.rjtOpin" alt="반려의견" /></td>
		<td>
			<u:checkArea>
				<u:input type="text"  style="width:98%;" id="returnSurvCont" name="returnSurvCont"  titleId="cols.rjtOpin" alt="반려의견" mandatory="Y"/>
			</u:checkArea>
		</td>
	</tr>	
...
<u:listArea>
	<c:forEach  var="wvsQue" items="${wvsQueList}"  varStatus="status">
		...
		<tr>
		<td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="width:100%;table-layout:fixed;">
								...
								<tr>
									...
									<td class="body_lt">
										...
										<c:if test="${wvsQueExam.inputYn == 'Y'}">
											<br><input id="inputRadio${wvsQueExam.quesId}" name ="inputRadio${wvsQueExam.quesId}" type="text" style="width:98%;" disabled="disabled"/>
...
<u:buttonArea>
	<u:button titleId="cm.btn.save" onclick="formSubmit();" alt="저장"/>
