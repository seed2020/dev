...
function setPopupPop() {
	dialog.open('setPopupDialog','<u:msg titleId="wv.btn.setPopup" alt="로그인 팝업설정" />','./setPopupPop.do?menuId=${menuId}&survId=${param.survId}');
}
//이미지 미리보기 팝업
	...
	<td><table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr class="head_rd">
			...
			<strong><u:msg titleId="cols.ques" alt="질문" /><u:out value="${status.count}) ${wvsQue.quesCont}"/></strong>
				...
				<c:if test="${!empty wvsQue.mulChoiYn}">
					<td class="listicon_rt"><u:buttonS href="javascript:dialog.open('listDeptStacsPop','부서별통계','../listDeptStacsPop.do?menuId=${menuId}&survId=${wvsQue.survId}&quesId=${wvsQue.quesId}&quesCount=${status.count}');" titleId="wv.btn.deptStacs" alt="부서별통계" /></td>
				</c:if>
				<c:if test="${empty wvsQue.mulChoiYn}">
					<td class="listicon_rt"><u:buttonS href="javascript:dialog.open('listOendAnsPop','주관식 답변 리스트','../listOendAnsPop.do?menuId=${menuId}&&survId=${wvsQue.survId}&quesId=${wvsQue.quesId}&quesCount=${status.count}');" titleId="wv.btn.resView" alt="결과보기" /></td>
				</c:if>
		</tr>
					...
					<div style="padding:5px 10px 5px 10px;">
						<table border="0" cellpadding="0" cellspacing="0" style="width:50%; table-layout:fixed;">
							<tr> 
								<td width="300" class="body_lt">
									<div class="ellipsis" title="${wvsQueExam.examOrdr}.${wvsQueExam.examDispNm}">
										<span class="wv_survInfo" id="wv_survInfo${queExStatus.count}">
											<u:out value="${wvsQueExam.examOrdr}.${wvsQueExam.examDispNm}" maxLength="47"/>
								...
								<td width="100" class="body_lt" rowspan="2">${wvsQueExam.selectCount}명 선택</td>
								...
								<td width="80" class="body_ct" rowspan="2">
									<c:if test="${wvsQueExam.inputYn == 'Y' }">
										<c:if test="${wvsQueExam.selectCount != 0}">
											<u:buttonS href="javascript:dialog.open('listMulcAnsPop','객관식 입력 항목 답변 내용','../listMulcAnsPop.do?menuId=${menuId}&survId=${wvsQueExam.survId}&quesId=${wvsQueExam.quesId}&replyNo=${wvsQueExam.examNo}');" titleId="wv.btn.resView" alt="결과보기" />
										</c:if>
									</c:if>
								</td>
							</tr>
							<c:if test="${!empty wvsQueExam.imgSavePath}">
								...
								<a href="javascript:viewSurvImagePop('${wvsQueExam.survId}','${wvsQueExam.quesId }','${wvsQueExam.examNo }');"><img src="${_ctx}${wvsQueExam.imgSavePath}" width="110" height="125" /></a></label></td>
							</c:if>
						</table>
					</div>
...
<u:buttonArea>
	<c:choose>
		<c:when test="${wvsVo.survPrgStatCd == '4'}">
			<u:button titleId="cm.btn.del" alt="삭제" href="javascript:survAdmDel('${wvsVo.survId}');" auth="R" />
			<u:button titleId="cm.btn.list" alt="목록" href="./listSurv.do?menuId=${menuId}" auth="R" />
		</c:when>
		<c:when test="${wvsVo.survPrgStatCd == '3'}">
			<u:button titleId="wv.btn.setPopup" alt="로그인 팝업설정" onclick="setPopupPop();" />
			<u:button titleId="wv.btn.endMod" alt="마감변경" href="javascript:dialog.open('setEndModPop','마감변경','./setEndModPop.do?menuId=${menuId}&survId=${wvsVo.survId}');" auth="R" />
			<u:button titleId="cm.btn.list" alt="목록" href="./listSurv.do?menuId=${menuId}" auth="R" />
		</c:when>
		<c:otherwise>
			<u:button titleId="cm.btn.list" alt="목록" href="./listSurv.do?menuId=${menuId}" auth="R" />
