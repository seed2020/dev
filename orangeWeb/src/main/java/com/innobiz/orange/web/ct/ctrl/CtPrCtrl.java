...
//	private OrCmSvc orCmSvc;
	...
	/** Logger */
	private static final Logger LOGGER = Logger.getLogger(CtPrCtrl.class);
	...
	public String listPr(HttpServletRequest request,
		...
		return LayoutUtil.getJspPath("/ct/pr/listPr");
...
//		}
		...
		// 최대 본문 사이즈 model에 추가
		ctPrSvc.putBodySizeToModel(request, model);
		
		// params
		model.addAttribute("params", ParamUtil.getQueryString(request, "bullId"));
		
		return LayoutUtil.getJspPath("/ct/pr/setPr");
		
	}
	
	/** 커뮤니티 홍보마당 저장 */
	@RequestMapping(value = "ct/pr/transSetCtPrSave")
	public String transSetCtPrSave(HttpServletRequest request, ModelMap model){
		...
		try{
			...
			if (goView) {
				model.put("todo", "parent.location.replace('/ct/pr/viewPr.do?menuId="+request.getParameter("menuId")+"&bullId="+bullId+"');");
			} else {
				model.put("todo", "parent.location.replace('/ct/pr/listPr.do?menuId="+request.getParameter("menuId")+"');");
			}
		}catch (CmException e) {
	...
	public String viewPr(HttpServletRequest request,
		...
		return LayoutUtil.getJspPath("/ct/pr/viewPr");
