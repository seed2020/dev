...
//	private PtCacheSvc ptCacheSvc;
	...
	public String createId(String tableName) throws SQLException {
		if ("WF_FORM_GRP_B".equals(tableName)) { // 양식 그룹
			return commonSvc.createId(tableName, 'G', 8);
		}else if ("WF_RESC_B".equals(tableName)) { // 리소스
			return commonSvc.createId(tableName, 'R', 8);
		}else if ("WF_FORM_COLM_L".equals(tableName)) { // 양식 컬럼
			return commonSvc.createId(tableName, 'C', 8);
		}else if ("WF_FORM_GEN_B".equals(tableName)) { // 양식 생성
			return commonSvc.createId(tableName, 'T', 8);
		}else if ("WF_CD_GRP_B".equals(tableName)) { // 코드그룹
			return commonSvc.createId(tableName, 'G', 8);
		}else if ("WF_CD_D".equals(tableName)) { // 코드그룹
			return commonSvc.createId(tableName, 'C', 8);
		}
	...
	public String toXmlDataType(String colmNm){
		
		if(colmNm.startsWith("number")){
			return "VARCHAR";
		}else if(colmNm.startsWith("text")){
			return "NVARCHAR";
		}else if(colmNm.startsWith("editor")){
			return "LONGVARCHAR";
		}else if(colmNm.startsWith("date") || colmNm.startsWith("period") || colmNm.startsWith("time") || colmNm.startsWith("calculate") || colmNm.startsWith("radio") || colmNm.startsWith("select") || colmNm.startsWith("checkbox")){
			return "VARCHAR";
		}else{
			return "VARCHAR";
		}
